// src/dashboard/components/primitives.tsx
// All stable style objects live at module level — recreating them inside
// render functions triggers unnecessary GC pressure on every repaint.
...
const AMBER_DIVIDER_BASE: React.CSSProperties = {
  height: 1,
  flexShrink: 0,
};
...
// ── Hole size cache — keyed by index so the %-based variation is stable ──
// We pre-compute up to 64 holes once and reuse them.
const MAX_HOLES = 64;
...
export const SprocketStrip = memo<SprocketStripProps>(({ count = 30, vertical = false }) => {
  // Slice from the pre-computed array — no allocation on re-render
...
const LEFT_EDGE_STYLE: React.CSSProperties = {
  ...
  width: 38, background: 'var(--film-dark)',
...
export const MarqueeTicker = memo<MarqueeTickerProps>(({ items, speed = 28 }) => {
  // doubled array is stable if items reference is stable (module-level constant)
