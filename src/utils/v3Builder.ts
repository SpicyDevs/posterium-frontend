...
export function buildOptimalUrl(config: PosterConfig, baseUrl = 'https://api.spicydevs.xyz'): string {
  ...
  config.ratings.forEach((key: RatingType, index: number) => {
    ...
    if (item.radius  !== undefined && item.radius  !== hoisted.radius)  p.set(`${sp}_ra`,  ps(item.radius));
    if (item.shadow  !== undefined && item.shadow  !== hoisted.shadow)  p.set(`${sp}_sh`,  ps(item.shadow));
    if (item.icon !== undefined && item.icon !== hoisted.icon)          p.set(`${sp}_ic`,  item.icon ? '1' : '0');
    if (item.scale   !== undefined && item.scale   !== hoisted.scale)   p.set(`${sp}_sc`,  item.scale.toFixed(3));
    if (item.borderW !== undefined)                                      p.set(`${sp}_bw`,  ps(item.borderW));
    if (item.borderC !== undefined && item.borderC !== hoisted.borderC) p.set(`${sp}_bc`,  item.borderC);
