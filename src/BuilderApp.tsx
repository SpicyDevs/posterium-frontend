...
const StudioLayout: React.FC<{
  ...
  // Stable refs so keyboard handler is registered only once
  ...
  // -------------------------------------------------------------------------
  // Mobile sheet — transform-only snap (no height reflow, no nested scroll breaks)
  // -------------------------------------------------------------------------
  ...
  // Translate values for each snap point
  const SNAPS = { hidden: '100%', half: '0%', full: '0%' } as const;

  // Sheet height for each mode — only the height changes, not the translate
  const HEIGHTS = {
  ...
  // Sync on mode change (triggered from code, not drag)
  useEffect(() => {
  ...
  const onHandleTouchMove = (e: React.TouchEvent) => {
    ...
    const el = sheetRef.current;
    if (!el) return;
    // If trying to drag past full, apply rubber-band resistance
  ...
  const onHandleTouchEnd = () => {
    ...
    // Animate sheet to target, then update state if changed
    snapTo(next);
      ...
      <div className="flex flex-col h-[100dvh] overflow-hidden font-sans bg-[#09090b] text-zinc-200 selection:bg-indigo-500/30">
        ...
        <header className="h-14 flex-shrink-0 flex items-center gap-2 px-3 border-b border-white/[0.06] bg-[#09090b] z-30">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <span className="hidden sm:block text-[13px] font-semibold text-white tracking-tight select-none">
              FreePosterAPI
            </span>
          </div>
        ...
        {/* ============================================================
            MOBILE DOCK (always visible on mobile)
        ============================================================ */}
        <MobileDock />
        ...
        {/* ============================================================
            ...
            Architecture:
            - Fixed above dock (bottom = dock height + safe-area)
            - transform-only snapping — no height transition = no reflow
            - Content uses touch-action: pan-y + overflow-y: auto
              so native scroll is NEVER blocked by the sheet
            - Drag handle has touch-action: none (only where needed)
            - pointer-events: none when hidden (prevents ghost taps on canvas)
        ============================================================ */}
        <div
          ref={sheetRef}
          role="complementary"
        ...
        >
          {/* Drag handle — touch-none scoped only here */}
          <div
          ...
          {/* Content — fully scrollable, never blocked */}
          <div
