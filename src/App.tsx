...
import { Sparkles, Github, RotateCcw, Download } from 'lucide-react';
    ...
    <div className="flex flex-col h-[100dvh] bg-[#0a0a0a] text-zinc-200 overflow-hidden font-sans">
      <header className="h-14 flex-shrink-0 flex items-center justify-between gap-4 px-4 border-b border-white/5 bg-[#0a0a0a] z-30">
        <div className="flex items-center gap-2.5 flex-shrink-0">
          ...
          <h1 className="hidden sm:block text-base font-bold tracking-tight text-white">FreePosterAPI</h1>
        </div>
        ...
        <div className="flex gap-2 items-center flex-shrink-0">
             <button 
                ...
                className="flex items-center gap-2 px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded-md text-xs font-semibold transition-colors shadow-lg shadow-blue-900/20"
             >
                <Download size={14} />
                <span className="hidden sm:inline">Download</span>
             </button>
             <div className="w-px h-5 bg-white/10 mx-1"></div>
             <button onClick={handleReset} className="p-2 text-red-400 bg-red-400/10 hover:bg-red-400/20 rounded-md transition-colors" title="Reset"><RotateCcw size={16} /></button>
