...
const DraggableBadge: React.FC<Props> = ({ badgeId, config, x, y, canvasScale, onPositionChange }) => {
  ...
  // Handle Background (Solid vs Gradient)
  ...
  const renderContent = () => {
    ...
    if (!showIcon) {
        return (
            <span style={{ 
                position: 'absolute', left: '50%', top: textTop, transform: 'translate(-50%, -50%)',
                fontSize: `${28 * scale}px`, fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 'bold',
        ...
        return (
             <>
                <div style={{ position: 'absolute', left: iconLeft, top: 14 * scale }}>
                    <svg viewBox={ICONS.runtime.vb} width={iconSize} height={iconSize} style={{ display: 'block' }} dangerouslySetInnerHTML={{ __html: ICONS.runtime.body }} color={txtColor}/>
                </div>
                <span style={{ 
                    position: 'absolute', right: textRight, top: textTop, transform: 'translateY(-50%)',
                    fontSize: `${28 * scale}px`, fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 'bold',
                    color: txtColor, lineHeight: 1 
                }}>
                    2h 15m
    ...
    if (badgeId === 'rt') iconKey = 'rt_fresh';
    if (badgeId === 'rt_popcorn') iconKey = 'popcorn_fresh';
  ...
  return (
    <div
      ...
      style={{
        width: `${width}px`,
        height: `${height}px`,
        transform: `translate(${currentPos.x}px, ${currentPos.y}px) scale(${itemScale})`, // ADDED SCALE HERE
        background: backgroundStyle, // ADDED GRADIENT SUPPORT
        borderRadius: `${radiusVal}px`,
        border: borderWidth > 0 ? `${borderWidth}px solid ${borderColor}` : 'none', // ADDED BORDER
    ...
    >
      <div className="opacity-0 group-hover:opacity-100 absolute -left-8 top-1/2 -translate-y-1/2 bg-blue-600/90 backdrop-blur rounded-full p-2 text-white transition-opacity shadow-lg" style={{ transform: `scale(${1/itemScale})` }}>
