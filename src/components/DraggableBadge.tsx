...
const DraggableBadge: React.FC<Props> = ({ badgeId, config, x, y, canvasScale, onPositionChange, obstacles }) => {
  ...
  const handleMove = (clientX: number, clientY: number) => {
    const screenDeltaX = clientX - dragOffset.x;
    const screenDeltaY = clientY - dragOffset.y;

    const canvasDeltaX = screenDeltaX / canvasScale;
    const canvasDeltaY = screenDeltaY / canvasScale;

    let newX = currentPos.x + canvasDeltaX;
    let newY = currentPos.y + canvasDeltaY;
    ...
    // 1. Boundary Check
    newX = Math.max(0, Math.min(newX, CANVAS_WIDTH - widthScaled));
    newY = Math.max(0, Math.min(newY, CANVAS_HEIGHT - heightScaled));

    // 2. Collision Check (Simple Blocking)
    // If the new position overlaps any obstacle, we don't move there.
    // However, strictly blocking movement can feel stuck. 
    // A better approach for "can only touch" is to allow movement up to the edge.
    // For simplicity in this React implementation, we will check if the move is valid.
    // If invalid, we revert to the last valid position on that axis (trying X and Y independently).
    
    const xValid = !checkCollision(newX, currentPos.y, widthScaled, heightScaled);
    const yValid = !checkCollision(currentPos.x, newY, widthScaled, heightScaled);
    const bothValid = !checkCollision(newX, newY, widthScaled, heightScaled);

    if (bothValid) {
        // Move freely
    } else if (xValid) {
        newY = currentPos.y; // Block Y movement
    } else if (yValid) {
        newX = currentPos.x; // Block X movement
    } else {
        // Block all movement
        newX = currentPos.x;
        newY = currentPos.y;
    ...
    // 3. Grid Snapping (Only if not colliding)
    const snapThreshold = 10;
    const centerX = newX + widthScaled / 2;
    const centerY = newY + heightScaled / 2;
    ...
    for (const line of vSnaps) {
      if (Math.abs(centerX - line) < snapThreshold) {
          const snappedX = line - widthScaled / 2;
          if (!checkCollision(snappedX, newY, widthScaled, heightScaled)) {
              newX = snappedX;
          }
      }
    }
    for (const line of hSnaps) {
      if (Math.abs(centerY - line) < snapThreshold) {
          const snappedY = line - heightScaled / 2;
          if (!checkCollision(newX, snappedY, widthScaled, heightScaled)) {
              newY = snappedY;
          }
      }
    }
