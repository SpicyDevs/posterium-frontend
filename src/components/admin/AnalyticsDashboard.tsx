...
const PIE_COLORS = [CH.amber, CH.blue, CH.green, CH.yellow, CH.orange, CH.purple, CH.teal, CH.red, CH.pink];

function nodeColor(n: string) {
  const MAP: Record<string, string> = {
    washington: CH.blue, london: CH.green, tokyo: CH.yellow, mumbai: CH.red,
    germany: CH.teal, france: CH.purple, wsrv: CH.orange, 'render-eu': CH.pink,
    ohio: CH.purple, 'cf-binding': CH.gold, 'render_eu': CH.pink,
  };
...
function nodeLabel(n: string) {
  const MAP: Record<string, string> = {
    ...
    germany: 'Germany · Spaceify', france: 'France · Spaceify',
    wsrv: 'wsrv.nl', 'render-eu': 'EU Central · Render',
    'cf-binding': 'CF Binding', 'render_eu': 'EU Central · Render',
  };
  return MAP[n] ?? n;
}
...
function fmtBucket(s: string, granularity: string) {
  ...
  try {
    ...
    if (granularity === 'hour')  return `${d.getUTCMonth()+1}/${d.getUTCDate()} ${String(d.getUTCHours()).padStart(2,'0')}h`;
...
const TABS = ['overview', 'nodes', 'traffic', 'fallbacks', 'requests', 'devices', 'db', 'errors', 'breakdown', 'wall-time'] as const;
    ...
    <div style={{
      ...
      {payload.map((p: any, i: number) => (
        <div key={i} style={{ color: p.color ?? CH.cream, marginBottom: 3, display: 'flex', justifyContent: 'space-between', gap: 12 }}>
          ...
          <strong>{typeof p.value === 'number' ? (p.value > 10000 ? fmtNum(p.value) : p.value % 1 !== 0 ? p.value.toFixed(1) : p.value) : p.value}</strong>
        </div>
      ))}
    </div>
  );
...
const StatCard = ({ label, value, sub, color = CH.amber, alert }: {
  ...
  <div style={{
    ...
    {alert && <div style={{
      ...
      background: CH.red, boxShadow: `0 0 6px ${CH.red}`, animation: 'pulse-dot 2s ease-in-out infinite',
    }} />}
...
const NodeCard = ({ row, latRow, alertRate, alertMs }: any) => {
  ...
  return (
    <div style={{ padding: '12px 14px', background: 'var(--film-char)', border: '1px solid var(--film-border)',
      ...
      {alertMs_ && <div style={{ position: 'absolute', top: 8, right: 8, width: 6, height: 6,
...
const PosterThumb = ({ id, type, hits, hitRate }: { id: string; type: string; hits: number; hitRate: number }) => {
  const [loaded, setLoaded] = useState(false);
  const [errored, setErrored] = useState(false);
  const src = `${POSTER_API}/${type}/${id}.svg?source=tmdb`;
  return (
    <a href={`https://api.spicydevs.xyz/test/${type}/${id}.png`} target="_blank" rel="noreferrer"
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 7,
            color: rateColor(hitRate), background: 'rgba(0,0,0,0.7)', padding: '1px 4px', borderRadius: 2 }}>
      ...
      <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
        <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 7,
          ...
          background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)',
          borderRadius: 2, padding: '1px 4px', textTransform: 'uppercase' as const }}>{type}</span>
        <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 8, color: CH.dim,
          overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const }}>{id}</span>
      </div>
    </a>
  );
};
    ...
    <div style={{ minHeight: '100dvh', background: 'var(--film-black)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{
        ...
        <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
          ...
          <div className="poster-font" style={{ fontSize: 28, color: 'var(--film-cream)', letterSpacing: '0.06em' }}>Dashboard v4</div>
          <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 8, color: CH.ghost, marginTop: 4 }}>raster_metrics + request_analytics + D1</div>
        </div>
...
export default function AnalyticsDashboard() {
  ...
  // ── Derived data ───────────────────────────────────────────────────────────
  const nodeRows = useMemo(() => (data?.data?.node_performance?.data ?? []).map((r: any) => ({
  ...
  const trafficData = useMemo(() => (data?.data?.traffic_timeseries?.data ?? []).map((r: any) => ({
    bucket: mkBucket(r.bucket ?? ''), attempts: num(r.attempts), successes: num(r.successes),
    failures: num(r.failures), avg_ms: num(r.avg_ms),
  })), [data, mkBucket]);

  const latencyRows = useMemo(() => (data?.data?.latency_percentiles?.data ?? []).map((r: any) => ({
  ...
  const failRows = useMemo(() => (data?.data?.recent_failures?.data ?? []).map((r: any) => ({
    node: String(r.node ?? ''), error: String(r.error ?? ''),
    status_code: num(r.status_code), timestamp: String(r.timestamp ?? ''),
  })), [data]);

  const fallbackTierRows = useMemo(() => (data?.data?.fallback_tiers?.data ?? [])
  ...
  const fallbackTimeseries = useMemo(() => (data?.data?.fallback_timeseries?.data ?? []).map((r: any) => ({
  ...
  const topIds = useMemo(() => (data?.data?.req_top_ids?.data ?? []).map((r: any) => ({
    id: String(r.id ?? ''), type: String(r.type ?? 'movie'),
    hits: num(r.hits), hit_rate_pct: num(r.hit_rate_pct),
  })), [data]);

  const deviceRows = useMemo(() => (data?.data?.req_device_breakdown?.data ?? []).map((r: any) => ({
    device: String(r.device ?? ''), requests: num(r.requests), cache_hits: num(r.cache_hits),
  })), [data]);

  const countryRows = useMemo(() => (data?.data?.req_country_breakdown?.data ?? []).map((r: any) => ({
    country: String(r.country ?? ''), requests: num(r.requests), cache_hits: num(r.cache_hits),
  })), [data]);

  const reqTimeseries = useMemo(() => (data?.data?.req_timeseries?.data ?? []).map((r: any) => ({
    bucket: mkBucket(r.bucket ?? ''), requests: num(r.requests),
    hits: num(r.hits), misses: num(r.misses),
  })), [data, mkBucket]);

  const cacheTimeseries = useMemo(() => (data?.data?.req_cache_timeseries?.data ?? []).map((r: any) => ({
    bucket: mkBucket(r.bucket ?? ''), total: num(r.total),
    hits: num(r.hits), hit_rate_pct: num(r.hit_rate_pct),
  })), [data, mkBucket]);

  const reqFormatRows = useMemo(() => (data?.data?.req_format_breakdown?.data ?? []).map((r: any) => ({
    format: String(r.format ?? ''), requests: num(r.requests),
  })), [data]);

  const reqTypeRows = useMemo(() => (data?.data?.req_type_breakdown?.data ?? []).map((r: any) => ({
    type: String(r.type ?? ''), requests: num(r.requests),
  })), [data]);

  const topRatings = useMemo(() => (data?.data?.req_top_ratings?.data ?? []).map((r: any) => ({
  ...
  return (
    <div style={{ minHeight: '100dvh', background: 'var(--film-black)', color: 'var(--film-cream)', paddingTop: 56 }}>
      ...
      {/* ── Sticky controls ─────────────────────────────────────────────── */}
      <div style={{
        position: 'sticky', top: 56, zIndex: 40, background: 'rgba(7,7,6,0.97)',
        ...
        <button onClick={() => fetchData()} disabled={loading} style={{
          height: 28, padding: '0 12px', background: loading ? 'rgba(196,124,46,0.3)' : CH.amber,
      ...
      <nav style={{ background: 'var(--film-dark)', borderBottom: '1px solid rgba(255,255,255,0.05)',
        ...
        {TABS.map(t => (
          <button key={t} onClick={() => setTab(t)} style={{
            ...
            {t === 'wall-time' ? 'Wall Time' : t === 'fallbacks' ? 'Fallbacks' :
             t === 'requests' ? '★ Requests' : t === 'devices' ? '★ Devices' : t === 'db' ? '★ DB Stats' : t}
          </button>
        ))}
      </nav>
        ...
        {tab === 'overview' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(210px,1fr))', gap: 10 }}>
              {loading ? Array(4).fill(0).map((_,i) => <Skel key={i} h={120}/>) : nodeRows.map(row => (
              ...
              <div style={{ overflowX: 'auto', padding: 14 }}>
                {loading ? <Skel h={300}/> : (
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                    <thead><tr>{['Node','Health','Attempts','Success Rate','Avg Latency','Wins','Score'].map(h => (
                    ...
                    <tbody>{nodeRows.map((row, i) => {
                      ...
                      return (
                        <tr key={row.node} style={{ background:i%2===0?'rgba(255,255,255,0.012)':'transparent', borderBottom:'1px solid rgba(255,255,255,0.03)' }}>
                          <td style={{ padding:'9px 12px' }}><div style={{ display:'flex',alignItems:'center',gap:6 }}>
                          ...
                          <td style={{ padding:'9px 12px' }}><span style={{ fontFamily:'JetBrains Mono,monospace',fontSize:7,color:hc,textTransform:'uppercase' as const,background:`${hc}18`,padding:'2px 6px',borderRadius:3 }}>{health}</span></td>
              ...
              {loading ? <Skel h={200}/> : (
                <ResponsiveContainer width="100%" height={200}>
                  <ComposedChart data={reqTimeseries} margin={{ top:4,right:4,bottom:0,left:0 }}>
                    <defs>
                      <linearGradient id="gReq" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={CH.teal} stopOpacity={0.25}/><stop offset="95%" stopColor={CH.teal} stopOpacity={0}/></linearGradient>
                    </defs>
                    <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)"/>
                    <XAxis dataKey="bucket" tick={{ fill:CH.ghost,fontSize:8 }} tickLine={false} axisLine={false} interval="preserveStartEnd"/>
                    <YAxis tick={{ fill:CH.ghost,fontSize:8 }} tickLine={false} axisLine={false} width={42}/>
            ...
            <Card title="Top Requested Posters" tag={`top ${topIds.length}`}>
              {loading ? <Skel h={320}/> : topIds.length === 0 ? (
                ...
                <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(110px,1fr))', gap:12 }}>
                  {topIds.map(row => (
                    <PosterThumb key={`${row.type}-${row.id}`} id={row.id} type={row.type}
                      hits={row.hits} hitRate={row.hit_rate_pct} />
                  ))}
                </div>
              )}
            </Card>
                      ...
                      return (
                        <div key={r.r_param}>
                          <div style={{ display:'flex',justifyContent:'space-between',marginBottom:3 }}>
                            <code style={{ fontFamily:'JetBrains Mono,monospace',fontSize:9,color:CH.amber,
                              background:'rgba(196,124,46,0.08)',border:'1px solid rgba(196,124,46,0.14)',
                              borderRadius:3,padding:'1px 5px' }}>{r.r_param}</code>
        ...
        {tab === 'devices' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ padding: '6px 0' }}>
              ...
              <p className="body-font" style={{ fontSize: 13, color: 'var(--film-text-dim)', marginTop: 8 }}>
                Device type breakdown inferred from User-Agent headers. Smart TV, mobile, tablet, and desktop classification.
              </p>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(160px,1fr))', gap: 10 }}>
              {loading ? Array(4).fill(0).map((_,i) => <Skel key={i} h={90}/>) : (
                ['desktop','mobile','tablet','tv'].map(dev => {
                  ...
                  return (
                    <StatCard key={dev} label={`${meta.icon} ${meta.label}`}
                      value={fmtNum(reqs)} sub={`${fmtPct(pct)} of traffic`} color={meta.color} />
                  );
                })
              )}
            </div>
                ...
                {loading ? <Skel h={260}/> : deviceRows.length === 0 ? (
                  ...
                  <div style={{ display:'flex',flexDirection:'column',gap:12 }}>
                    <ResponsiveContainer width="100%" height={180}>
                      <PieChart>
                        <Pie data={deviceRows} cx="50%" cy="50%" innerRadius={50} outerRadius={80}
                          dataKey="requests" nameKey="device" paddingAngle={2}>
                          {deviceRows.map((r, i) => (
                            <Cell key={r.device} fill={DEVICE_META[r.device]?.color ?? PIE_COLORS[i % PIE_COLORS.length]} />
                          ))}
                        </Pie>
              ...
              <Card title="Media Type by Requests">
                {loading ? <Skel h={260}/> : (
                  <div style={{ display:'flex',flexDirection:'column',gap:12 }}>
                    <ResponsiveContainer width="100%" height={180}>
                      <PieChart>
                        <Pie data={reqTypeRows} cx="50%" cy="50%" innerRadius={50} outerRadius={80}
                          dataKey="requests" nameKey="type" paddingAngle={2}>
                          {reqTypeRows.map((r, i) => (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ padding: '6px 0' }}>
              ...
              <p className="body-font" style={{ fontSize: 13, color: 'var(--film-text-dim)', marginTop: 8 }}>
                ...
                Queried directly from D1 on each analytics request; no counters written.
              </p>
            </div>

            {dbStats?.error ? (
              <div style={{ padding:'16px', borderRadius:8, background:'rgba(248,113,113,0.07)', border:'1px solid rgba(248,113,113,0.2)', color:CH.red, fontFamily:'JetBrains Mono,monospace', fontSize:11 }}>
                  ...
                  {loading ? <Skel h={160}/> : (
                    <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))',gap:10 }}>
                        ...
                        return (
                          <div key={r.type} style={{ padding:'12px 14px',background:'var(--film-char)',border:'1px solid var(--film-border)',borderRadius:8 }}>
                            <div style={{ display:'flex',justifyContent:'space-between',marginBottom:8 }}>
                              ...
                              <span style={{ fontFamily:'JetBrains Mono,monospace',fontSize:10,color:PIE_COLORS[i%PIE_COLORS.length],fontWeight:700 }}>
                                {fmtPct(num(r.count)/total*100)}
                              </span>
                            </div>
                            <div style={{ fontFamily:'JetBrains Mono,monospace',fontSize:20,color:PIE_COLORS[i%PIE_COLORS.length],fontWeight:700,marginBottom:8 }}>
                              {fmtNum(num(r.count))}
                            </div>
                            <div style={{ height:4,borderRadius:2,background:'rgba(255,255,255,0.06)',overflow:'hidden' }}>
                      ...
                      {(dbStats.byType ?? []).length === 0 && (
                        <div style={{ color:CH.ghost,fontFamily:'JetBrains Mono,monospace',fontSize:11 }}>No type data</div>
                      )}
                    </div>
                  )}
                </Card>
                      ...
                      {[
                        ...
                        return (
                          <div key={band.key} style={{ padding:'12px 14px',background:'var(--film-char)',border:'1px solid var(--film-border)',borderRadius:8 }}>
                            ...
                            <div style={{ fontFamily:'JetBrains Mono,monospace',fontSize:7,color:CH.ghost }}>
                  ...
                  <table style={{ width:'100%',borderCollapse:'collapse',fontSize:10 }}>
                    ...
                    <tbody>{failRows.slice(0,100).map((r,i)=>(
                      <tr key={i} style={{ background:i%2===0?'rgba(248,113,113,0.025)':'transparent',borderBottom:'1px solid rgba(255,255,255,0.025)' }}>
                        ...
                        <td style={{ padding:'6px 12px' }}><div style={{ display:'flex',alignItems:'center',gap:5 }}>
                        ...
                        <td style={{ padding:'6px 12px' }}><span style={{ fontFamily:'JetBrains Mono,monospace',fontSize:8,color:r.status_code===0?CH.orange:CH.red,background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:3,padding:'1px 5px' }}>
