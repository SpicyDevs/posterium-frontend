        ...
        {getVsPost.length > 0 && (
          <div style={{ padding: '10px 12px', background: 'var(--film-char)', borderRadius: 7, borderLeft: `3px solid ${CH.green}` }}>
            ...
            <div style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: 7, color: CH.ghost, marginTop: 3 }}>
              {getVsPost[0]?.delta > 0
                ? 'Prefer GET for URL-payload capable nodes'
                : 'POST is competitive for this poster'}
            </div>
          </div>
        )}
      </div>
...
const TimingChart: React.FC<{ bench: Benchmark }> = ({ bench }) => {
  const data = bench.nodes
    ...
    .sort((a, b) =>
      Math.min(a.GET ?? Infinity, a.PostURL ?? Infinity, a.PostB64 ?? Infinity) -
      Math.min(b.GET ?? Infinity, b.PostURL ?? Infinity, b.PostB64 ?? Infinity)
    );
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 10, fontFamily: 'JetBrains Mono,monospace' }}>
        <thead>
          <tr style={{ background: 'var(--film-mid)' }}>
            {['Node', 'Mode', 'Via', 'Status', 'Time', 'Note'].map(h => (
              <th key={h} style={{
                padding: '7px 12px', textAlign: 'left', fontSize: 7, color: CH.ghost,
                letterSpacing: '0.14em', textTransform: 'uppercase' as const,
        ...
        <tbody>
          {rows.map((r, i) => (
            <tr key={i} style={{
              ...
              <td style={{ padding: '5px 12px' }}>
                {r.http
                  ? <span style={{ color: CH.teal, fontSize: 8 }}>proxy</span>
                  : <span style={{ color: CH.ghost, fontSize: 8 }}>direct</span>}
              </td>
              <td style={{ padding: '5px 12px' }}>
                <span style={{ color: r.ok ? CH.green : CH.red, fontWeight: 700 }}>
                  {r.ok ? '✓ OK' : '✗ FAIL'}
                </span>
              </td>
              ...
              <td style={{
                padding: '5px 12px', color: CH.ghost, maxWidth: 260,
          ...
          <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', background: 'rgba(45,212,191,0.03)' }}>
            ...
            <td style={{ padding: '5px 12px', color: CH.ghost, fontSize: 8 }}>direct</td>
            <td style={{ padding: '5px 12px' }}>
              <span style={{ color: bench.wsrv.ok ? CH.green : CH.red, fontWeight: 700 }}>
                {bench.wsrv.ok ? '✓ OK' : '✗ FAIL'}
              </span>
            </td>
            <td style={{ padding: '5px 12px', color: bench.wsrv.ok ? msColor(bench.wsrv.ms) : CH.ghost, fontWeight: bench.wsrv.ok ? 700 : 400 }}>
              {bench.wsrv.ok ? `${bench.wsrv.ms}ms` : '—'}
            </td>
...
const LoadingView: React.FC<{ type: string; id: string; step: string }> = ({ type, id, step }) => (
  <div style={{
    ...
    <div className="poster-font" style={{ fontSize: 20, color: CH.amber, letterSpacing: '0.06em', marginBottom: 14 }}>
      {type}/{id}
    </div>
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4, alignItems: 'center' }}>
      {STEPS.map((s, i) => {
        ...
        return (
          <div key={s} style={{
            fontFamily: 'JetBrains Mono,monospace', fontSize: 9, transition: 'color 0.3s',
            color: i < cur ? CH.green : s === step ? CH.cream : CH.ghost,
          }}>
            {i < cur ? '✓ ' : s === step ? '› ' : '  '}{s}
          </div>
        );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, maxWidth: 560 }}>
      <div style={{ display: 'grid', gridTemplateColumns: '110px 1fr 90px', gap: 10 }}>
        {[
          ...
          { label: 'ID (TMDB / IMDb / MAL)', el: (
            <input
              value={id} onChange={e => setId(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && run()}
              placeholder="e.g. tt1375666" style={base}
            />
          )},
          { label: 'Format', el: (
          ...
          <div key={label}>
            <div style={{
              ...
              letterSpacing: '0.16em', textTransform: 'uppercase' as const, marginBottom: 5,
            }}>
              {label}
            </div>
            {el}
      ...
      <button
        ...
        disabled={!id.trim()}
        style={{
          height: 42, background: `linear-gradient(90deg,${CH.amber},${CH.gold})`,
          color: '#070706', border: 'none', borderRadius: 8,
          cursor: id.trim() ? 'pointer' : 'not-allowed', fontSize: 12, fontWeight: 800,
          fontFamily: 'Syne,sans-serif', letterSpacing: '0.12em',
          textTransform: 'uppercase' as const, opacity: id.trim() ? 1 : 0.5,
        }}
      >
        Run Benchmark
      </button>
        ...
        {EXAMPLES.map(ex => (
          <button
            ...
            style={{
              ...
              color: CH.ghost, fontSize: 9, cursor: 'pointer',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <span style={{
          ...
          {bench.format.toUpperCase()}
        </span>
        <button
          onClick={onRerun}
          style={{
            marginLeft: 'auto', padding: '5px 12px', background: CH.amber, border: 'none',
            borderRadius: 6, color: '#070706', fontSize: 10, cursor: 'pointer',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(130px,1fr))', gap: 8 }}>
        {[
          ...
          { l: 'Nodes OK',     v: `${bench.summary.successCount}/${NODES.length}`,               c: CH.cream },
          ...
          <div key={l} style={{
            padding: '10px 12px', background: 'var(--film-char)',
            border: '1px solid var(--film-border)', borderRadius: 8,
          }}>
            <div style={{
              fontFamily: 'JetBrains Mono,monospace', fontSize: 7, color: CH.ghost,
              letterSpacing: '0.12em', textTransform: 'uppercase' as const, marginBottom: 5,
            }}>
              {l}
            </div>
      ...
      <div style={{ background: 'var(--film-mid)', border: '1px solid var(--film-border)', borderRadius: 10, overflow: 'hidden' }}>
        <div style={{
          ...
          fontFamily: 'Syne,sans-serif', fontSize: 9, fontWeight: 700,
          letterSpacing: '0.14em', textTransform: 'uppercase' as const, color: CH.amber,
          display: 'flex', gap: 10, alignItems: 'center',
        }}>
          Render Timing
          {[{ k: 'GET', c: CH.green }, { k: 'POST URL-SVG', c: CH.blue }, { k: 'POST B64-SVG', c: CH.orange }].map(({ k, c }) => (
            <span key={k} style={{
              display: 'flex', alignItems: 'center', gap: 4, fontSize: 8, color: CH.ghost,
              fontWeight: 400, letterSpacing: 0, textTransform: 'none' as const,
            }}>
      ...
      {tierGroups.map(([tier, nodes]) => {
        const directCount = nodes.filter(n => !n.http).length;
        const proxiedCount = nodes.filter(n => n.http).length;
        return (
          <div key={tier}>
            <div style={{
              fontSize: 9, fontWeight: 700, letterSpacing: '0.12em',
              textTransform: 'uppercase' as const,
              color: TIER_META[Number(tier)]?.color ?? CH.ghost,
              ...
              {directCount > 0 && <span>{directCount} direct</span>}
              {proxiedCount > 0 && (
                <>
                  <span style={{ color: 'rgba(255,255,255,0.15)', fontSize: 7 }}>·</span>
                  <span style={{ color: CH.teal }}>{proxiedCount} via proxy</span>
                </>
              )}
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(185px,1fr))', gap: 10 }}>
              {nodes.map(n => <NodeCard key={`${n.id}-${n.label}`} result={n} fastestMs={bench.summary.fastestMs} />)}
            </div>
          </div>
        );
      })}
      ...
      {bench.wsrv.imageUrl && (
        <div>
          <div style={{
            fontSize: 9, fontWeight: 700, letterSpacing: '0.12em',
            textTransform: 'uppercase' as const, color: CH.ghost,
            marginBottom: 10, paddingBottom: 6,
            borderBottom: '1px solid rgba(255,255,255,0.05)',
            fontFamily: 'JetBrains Mono,monospace',
          }}>
            Third-party · wsrv.nl (librsvg)
          </div>
          <div style={{
            maxWidth: 200, background: 'var(--film-char)',
            border: '1px solid var(--film-border)', borderLeft: `3px solid ${CH.teal}`,
            borderRadius: 8, overflow: 'hidden',
          }}>
            <div style={{
              padding: '9px 12px', borderBottom: '1px solid rgba(255,255,255,0.05)',
              ...
              <span style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: 10, fontWeight: 700, color: msColor(bench.wsrv.ms) }}>
                {bench.wsrv.ms}ms
              </span>
            </div>
      ...
      {/* SVG URLs */}
      <div style={{
        padding: 14, background: 'var(--film-char)', border: '1px solid var(--film-border)',
        ...
        {[
          ...
          <div key={label}>
            ...
            <a
              ...
              style={{
                fontFamily: 'JetBrains Mono,monospace', fontSize: 8, color: CH.amber,
                wordBreak: 'break-all' as const, textDecoration: 'none', lineHeight: 1.6,
              }}
            >
              {href}
            </a>
      ...
      <div style={{
        ...
        Completed {new Date(bench.timestamp).toLocaleTimeString()} · client-side via performance.now()
        · HTTP nodes routed via CF Worker proxy · blob images auto-revoked on next run
      </div>
    </div>
  );
};
  ...
  return (
    <div style={{ minHeight: '100dvh', background: 'var(--film-black)', color: 'var(--film-cream)', paddingTop: 56 }}>
      <style>{`
        ...
        .recharts-text { font-family: 'JetBrains Mono', monospace !important; font-size: 9px !important; fill: rgba(140,130,112,0.45) !important; }
      `}</style>
      ...
      <div style={{
        ...
        <div className="poster-font" style={{ fontSize: 15, color: CH.amber, letterSpacing: '0.06em' }}>
        ...
        <span style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: 7, color: CH.ghost }}>
          {mode === 'loading'
            ...
            : mode === 'results'
              ? 'RESULTS · GET + POST URL + POST B64'
              : 'READY · HTTP nodes via CF proxy'}
        </span>
      </div>
      ...
      <main style={{ padding: 16, maxWidth: 1280, margin: '0 auto' }}>
        ...
        {mode === 'search' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <div>
              <AmberTag style={{ marginBottom: 8 }}>Client-Side Node Benchmark v3</AmberTag>
              <p className="body-font" style={{ fontSize: 13, color: 'var(--film-text-dim)', maxWidth: 600, lineHeight: 1.7 }}>
                Tests all rasterizer nodes with 3 modes: <strong>GET ?url=</strong>, <strong>POST URL-SVG</strong>, <strong>POST B64-SVG</strong>.
                HTTP nodes (Spaceify, Midas) are now benchmarked via the CF Worker proxy at{' '}
                <code style={{ fontSize: 11, color: CH.teal }}>r-cf.spicydevs.xyz/proxy</code> — no mixed-content issues, Always-HTTPS stays on.
              </p>
            </div>
            <SearchForm onRun={go} />
          </div>
