...
// ... ICONS constant remains same as previous version or imported ...
// Re-declaring minimal necessary for BadgeIcon component to work if file is self-contained
const BADGE_ICONS: Record<string, { vb: string, body: string }> = {
  // ... (Icons from previous message would go here, kept short for brevity)
...
const Controls: React.FC<Props> = ({ config, onChange }) => {
  ...
  const updateBadgeConfig = (id: RatingType, updates: Partial<BadgeConfig>) => {
      onChange({
          ...config,
          items: { ...config.items, [id]: { ...config.items[id], ...updates } }
      });
  };
  ...
  const handleLayoutChange = (layout: 'row' | 'col') => { onChange({ ...config, layout, items: {} }); };
  const handlePresetChange = (preset: PresetType) => { onChange({ ...config, preset, items: {} }); };
  const handleMediaTypeSwitch = (type: 'movie' | 'tv') => {
      if (config.mediaType === type) return;
      onChange({ ...config, mediaType: type, tmdbId: type === 'movie' ? MOVIE_DEFAULT_ID : TV_DEFAULT_ID });
  };

  // Badge Background Handlers
  const handleBgTypeChange = (isGrad: boolean) => {
      const currentBg = config.items[selectedBadge as RatingType]?.bg || '#000000';
      if (isGrad) {
          // Switch to Gradient
          if (!currentBg.startsWith('grad:')) updateBadgeConfig(selectedBadge as RatingType, { bg: `grad:${currentBg}:${currentBg}` });
      } else {
          // Switch to Solid
          if (currentBg.startsWith('grad:')) {
              const c = currentBg.split(':')[1];
              updateBadgeConfig(selectedBadge as RatingType, { bg: c });
          }
      }
  };

  const handleGradientColorChange = (index: 0 | 1, color: string) => {
      const colors = getGradientColors(config.items[selectedBadge as RatingType]?.bg);
      colors[index] = color;
      updateBadgeConfig(selectedBadge as RatingType, { bg: `grad:${colors[0]}:${colors[1]}` });
  };
      ...
      <div className="flex-1 overflow-y-auto">
        <Section title="Source Media" icon={<ImageIcon size={16} />}>
            <div className="flex gap-2 mb-2">
                 <button onClick={() => handleMediaTypeSwitch('movie')} className={`flex-1 flex items-center justify-center gap-2 py-2 rounded text-xs border transition-colors ${config.mediaType === 'movie' ? 'bg-blue-600 border-blue-500 text-white' : 'bg-zinc-800/50 border-zinc-700 text-zinc-400 hover:bg-zinc-800'}`}><Film size={14}/> Movie</button>
                 <button onClick={() => handleMediaTypeSwitch('tv')} className={`flex-1 flex items-center justify-center gap-2 py-2 rounded text-xs border transition-colors ${config.mediaType === 'tv' ? 'bg-blue-600 border-blue-500 text-white' : 'bg-zinc-800/50 border-zinc-700 text-zinc-400 hover:bg-zinc-800'}`}><Tv size={14}/> TV Show</button>
            </div>
            <div className="grid grid-cols-3 gap-2 mb-2">
                <div className="col-span-2"><input type="text" value={config.tmdbId} onChange={(e) => handleChange('tmdbId', e.target.value)} className="w-full bg-zinc-800 border border-zinc-700 rounded px-3 py-2 text-xs focus:ring-1 focus:ring-blue-500 outline-none text-zinc-200 placeholder-zinc-600" placeholder="TMDB / IMDb ID"/></div>
                <div className="col-span-1 relative"><select value={config.source} onChange={(e) => handleChange('source', e.target.value)} className={selectClass}><option value="tmdb">TMDB</option><option value="fanart">Fanart</option></select></div>
            </div>
            <div className="flex bg-zinc-800 rounded border border-zinc-700 p-1">{(['svg', 'jpg', 'webp', 'png'] as const).map((ext) => (<button key={ext} onClick={() => handleChange('extension', ext)} className={`flex-1 text-[10px] font-medium uppercase py-1.5 rounded transition-all ${config.extension === ext ? 'bg-blue-600 text-white shadow-sm' : 'text-zinc-400 hover:text-zinc-200 hover:bg-zinc-700'}`}>{ext}</button>))}</div>
        </Section>
        
        {/* NEW: Poster Effects */}
        <Section title="Poster Effects" icon={<Sparkles size={16} />}>
            <div className="space-y-4">
                <div className="flex items-center justify-between">
                    <label className="text-xs text-zinc-300">Grayscale</label>
                    <input type="checkbox" checked={config.grayscale} onChange={(e) => handleChange('grayscale', e.target.checked)} className="rounded bg-zinc-700 border-zinc-600 text-blue-500 w-4 h-4"/>
                </div>
                <div>
                    <div className="flex justify-between text-[10px] text-zinc-500 uppercase tracking-wider mb-1.5"><span>Background Blur</span><span>{config.posterBlur}px</span></div>
                    <input type="range" min="0" max="20" value={config.posterBlur} onChange={(e) => handleChange('posterBlur', parseInt(e.target.value))} className="w-full accent-blue-500 h-1.5 bg-zinc-700 rounded-lg appearance-none cursor-pointer"/>
                </div>
            </div>
        </Section>
        ...
        <Section title="Styling" icon={<Palette size={16} />}>
            ...
            {selectedBadge === 'global' ? (
                <div className="space-y-4">
                    <div><div className="flex justify-between text-[10px] text-zinc-500 uppercase tracking-wider mb-1.5"><span>Blur</span><span>{config.blur}px</span></div><input type="range" min="0" max="20" value={config.blur} onChange={(e) => handleChange('blur', parseInt(e.target.value))} className="w-full accent-blue-500 h-1.5 bg-zinc-700 rounded-lg appearance-none cursor-pointer"/></div>
                    <div><div className="flex justify-between text-[10px] text-zinc-500 uppercase tracking-wider mb-1.5"><span>Opacity</span><span>{Math.round(config.alpha * 100)}%</span></div><input type="range" min="0" max="1" step="0.1" value={config.alpha} onChange={(e) => handleChange('alpha', parseFloat(e.target.value))} className="w-full accent-blue-500 h-1.5 bg-zinc-700 rounded-lg appearance-none cursor-pointer"/></div>
                    <div><div className="flex justify-between text-[10px] text-zinc-500 uppercase tracking-wider mb-1.5"><span>Corner Radius</span><span>{config.radius}px</span></div><input type="range" min="0" max="30" value={config.radius} onChange={(e) => handleChange('radius', parseInt(e.target.value))} className="w-full accent-blue-500 h-1.5 bg-zinc-700 rounded-lg appearance-none cursor-pointer"/></div>
                    <div><label className="flex items-center gap-2 text-xs text-zinc-400 cursor-pointer pt-1"><input type="checkbox" checked={config.shadow} onChange={(e) => handleChange('shadow', e.target.checked)} className="rounded bg-zinc-700 border-zinc-600 text-blue-500 w-4 h-4"/>Drop Shadow</label></div>
                </div>
            ) : (
                <div className="space-y-3 p-3 bg-zinc-800/50 rounded border border-blue-500/20">
                    <p className="text-xs text-blue-300 mb-2 flex items-center gap-1">Editing <BadgeIcon type={selectedBadge} className="w-3 h-3"/> <strong>{selectedBadge.replace('rt_popcorn', 'Popcorn').toUpperCase()}</strong></p>
                    
                    {selectedBadge !== 'age' && (<div className="mb-3 border-b border-zinc-700/50 pb-3"><label className="flex items-center gap-2 text-xs text-zinc-300 cursor-pointer"><input type="checkbox" checked={config.items[selectedBadge]?.icon ?? true} onChange={(e) => updateBadgeConfig(selectedBadge, { icon: e.target.checked })} className="rounded bg-zinc-700 border-zinc-600 text-blue-500 w-4 h-4"/>Show Icon</label></div>)}

                    {/* Scale Slider */}
                    <div className="mb-3">
                         <div className="flex justify-between text-[10px] text-zinc-500 uppercase tracking-wider mb-1"><span>Scale</span><span>{config.items[selectedBadge]?.scale ?? 1.0}x</span></div>
                         <input type="range" min="0.5" max="2.0" step="0.1" value={config.items[selectedBadge]?.scale ?? 1.0} onChange={(e) => updateBadgeConfig(selectedBadge, { scale: parseFloat(e.target.value) })} className="w-full accent-blue-500 h-1.5 bg-zinc-700 rounded-lg appearance-none cursor-pointer"/>
                    </div>

                    {/* Border Controls */}
                    <div className="grid grid-cols-2 gap-2 mb-3">
                         <div>
                             <div className="flex justify-between text-[10px] text-zinc-500 uppercase tracking-wider mb-1"><span>Border</span><span>{config.items[selectedBadge]?.borderW ?? 0}px</span></div>
                             <input type="range" min="0" max="10" value={config.items[selectedBadge]?.borderW ?? 0} onChange={(e) => updateBadgeConfig(selectedBadge, { borderW: parseInt(e.target.value) })} className="w-full accent-blue-500 h-1.5 bg-zinc-700 rounded-lg appearance-none cursor-pointer"/>
                         </div>
                         <div>
                            <label className="text-[10px] text-zinc-500 mb-1 block">Border Color</label>
                            <input type="color" value={config.items[selectedBadge]?.borderC || '#ffffff'} onChange={(e) => updateBadgeConfig(selectedBadge, { borderC: e.target.value })} className="h-6 w-full rounded bg-transparent cursor-pointer border border-zinc-700"/>
                         </div>
                    </div>

                    {/* Background Controls (Gradient support) */}
                    <div className="mb-3">
                        <label className="text-[10px] text-zinc-500 mb-1 block">Background Style</label>
                        <div className="flex bg-zinc-900 rounded p-1 mb-2">
                             <button onClick={() => handleBgTypeChange(false)} className={`flex-1 text-[10px] py-1 rounded ${!isGradient(config.items[selectedBadge]?.bg) ? 'bg-zinc-700 text-white' : 'text-zinc-500'}`}>Solid</button>
                             <button onClick={() => handleBgTypeChange(true)} className={`flex-1 text-[10px] py-1 rounded ${isGradient(config.items[selectedBadge]?.bg) ? 'bg-zinc-700 text-white' : 'text-zinc-500'}`}>Gradient</button>
                        </div>
                        
                        {isGradient(config.items[selectedBadge]?.bg) ? (
                             <div className="grid grid-cols-2 gap-2">
                                 <div><input type="color" value={getGradientColors(config.items[selectedBadge]?.bg)[0]} onChange={(e) => handleGradientColorChange(0, e.target.value)} className="h-8 w-full rounded border-zinc-700"/></div>
                                 <div><input type="color" value={getGradientColors(config.items[selectedBadge]?.bg)[1]} onChange={(e) => handleGradientColorChange(1, e.target.value)} className="h-8 w-full rounded border-zinc-700"/></div>
                             </div>
                        ) : (
                             <input type="color" value={config.items[selectedBadge]?.bg || '#000000'} onChange={(e) => updateBadgeConfig(selectedBadge, { bg: e.target.value })} className="h-8 w-full rounded border-zinc-700"/>
                        )}
                    </div>
                    ...
                    <div className="space-y-3 pt-2 border-t border-zinc-700/50">
                        <div><div className="flex justify-between text-[10px] text-zinc-500 uppercase tracking-wider mb-1"><span>Blur</span><span>{config.items[selectedBadge]?.blur ?? config.blur}px</span></div><input type="range" min="0" max="20" value={config.items[selectedBadge]?.blur ?? config.blur} onChange={(e) => updateBadgeConfig(selectedBadge, { blur: parseInt(e.target.value) })} className="w-full accent-blue-500 h-1.5 bg-zinc-700 rounded-lg appearance-none cursor-pointer"/></div>
                        <div><div className="flex justify-between text-[10px] text-zinc-500 uppercase tracking-wider mb-1"><span>Opacity</span><span>{Math.round((config.items[selectedBadge]?.alpha ?? config.alpha) * 100)}%</span></div><input type="range" min="0" max="1" step="0.1" value={config.items[selectedBadge]?.alpha ?? config.alpha} onChange={(e) => updateBadgeConfig(selectedBadge, { alpha: parseFloat(e.target.value) })} className="w-full accent-blue-500 h-1.5 bg-zinc-700 rounded-lg appearance-none cursor-pointer"/></div>
                        <div><div className="flex justify-between text-[10px] text-zinc-500 uppercase tracking-wider mb-1"><span>Radius</span><span>{config.items[selectedBadge]?.radius ?? config.radius}px</span></div><input type="range" min="0" max="30" value={config.items[selectedBadge]?.radius ?? config.radius} onChange={(e) => updateBadgeConfig(selectedBadge, { radius: parseInt(e.target.value) })} className="w-full accent-blue-500 h-1.5 bg-zinc-700 rounded-lg appearance-none cursor-pointer"/></div>
                    </div>

                    <button onClick={() => updateBadgeConfig(selectedBadge, { bg: undefined, txt: undefined, blur: undefined, alpha: undefined, radius: undefined, shadow: undefined, icon: undefined, scale: undefined, borderW: undefined, borderC: undefined })} className="w-full mt-2 text-xs py-1.5 text-zinc-500 hover:text-white border border-zinc-700 rounded hover:bg-zinc-800 transition-colors">Reset Overrides</button>
                </div>
            )}
        </Section>

        <Section title="Layout & Presets" icon={<Layout size={16} />}>
            <div className="grid grid-cols-3 gap-1 bg-zinc-800 p-1 rounded border border-zinc-700 mb-3">{presets.map(p => (<button key={p.id} onClick={() => handlePresetChange(p.id)} className={`aspect-square flex items-center justify-center text-[10px] font-bold rounded hover:bg-zinc-600 transition-colors ${config.preset === p.id ? 'bg-blue-600 text-white shadow-sm' : 'text-zinc-500 bg-zinc-900/50'}`}>{p.label}</button>))}</div>
            <div className="flex gap-2"><button onClick={() => handleLayoutChange('col')} className={`flex-1 text-xs py-2 rounded border transition-colors ${config.layout === 'col' ? 'bg-blue-600/20 border-blue-500 text-blue-200' : 'border-zinc-700 text-zinc-400 hover:bg-zinc-800'}`}>Column</button><button onClick={() => handleLayoutChange('row')} className={`flex-1 text-xs py-2 rounded border transition-colors ${config.layout === 'row' ? 'bg-blue-600/20 border-blue-500 text-blue-200' : 'border-zinc-700 text-zinc-400 hover:bg-zinc-800'}`}>Row</button></div>
        </Section>
