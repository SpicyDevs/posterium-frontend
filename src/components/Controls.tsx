      ...
      <div className="flex-1 overflow-y-auto">
        <Section title="Source & Effects" icon={<ImageIcon size={16} />}>
            ...
            <div className="space-y-4 pt-2 border-t border-white/5">
                ...
                <div>
                    ...
                    <input type="range" min="0" max="20" value={config.posterBlur} onChange={(e) => onChange({...config, posterBlur: parseInt(e.target.value)})} className="w-full accent-blue-500 h-1.5 bg-zinc-700 rounded-lg appearance-none cursor-pointer"/>
                </div>
            </div>
        </Section>
        ...
        <Section title="Styling" icon={<Palette size={16} />}>
            ...
            <div className={`space-y-3 p-3 rounded border ${isGlobal ? 'bg-zinc-900/30 border-transparent' : 'bg-blue-500/5 border-blue-500/20'}`}>
                {/* Scale */}
                <div className="mb-3">
                     <div className="flex justify-between text-[10px] text-zinc-500 uppercase tracking-wider mb-1"><span>Scale</span><span>{getCurrentValue('scale', 'globalScale') as number}x</span></div>
                     <input type="range" min="0.5" max="2.0" step="0.1" value={getCurrentValue('scale', 'globalScale') as number} onChange={(e) => updateValue('scale', 'globalScale', parseFloat(e.target.value))} className="w-full accent-blue-500 h-1.5 bg-zinc-700 rounded-lg appearance-none cursor-pointer"/>
                </div>
                ...
                <div className="mb-3">
                     <div className="flex justify-between text-[10px] text-zinc-500 uppercase tracking-wider mb-1"><span>Border Width</span><span>{getCurrentValue('borderW', 'globalBorderW') as number}px</span></div>
                     <input type="range" min="0" max="10" value={getCurrentValue('borderW', 'globalBorderW') as number} onChange={(e) => updateValue('borderW', 'globalBorderW', parseInt(e.target.value))} className="w-full accent-blue-500 h-1.5 bg-zinc-700 rounded-lg appearance-none cursor-pointer"/>
                </div>

                {/* Common Props */}
                <div className="space-y-3 pt-2 border-t border-zinc-700/50">
                    <div><div className="flex justify-between text-[10px] text-zinc-500 uppercase tracking-wider mb-1"><span>Blur</span><span>{getCurrentValue('blur', 'blur') as number}px</span></div><input type="range" min="0" max="20" value={getCurrentValue('blur', 'blur') as number} onChange={(e) => updateValue('blur', 'blur', parseInt(e.target.value))} className="w-full accent-blue-500 h-1.5 bg-zinc-700 rounded-lg appearance-none cursor-pointer"/></div>
                    <div><div className="flex justify-between text-[10px] text-zinc-500 uppercase tracking-wider mb-1"><span>Opacity</span><span>{Math.round((getCurrentValue('alpha', 'alpha') as number) * 100)}%</span></div><input type="range" min="0" max="1" step="0.1" value={getCurrentValue('alpha', 'alpha') as number} onChange={(e) => updateValue('alpha', 'alpha', parseFloat(e.target.value))} className="w-full accent-blue-500 h-1.5 bg-zinc-700 rounded-lg appearance-none cursor-pointer"/></div>
                    <div><div className="flex justify-between text-[10px] text-zinc-500 uppercase tracking-wider mb-1"><span>Radius</span><span>{getCurrentValue('radius', 'radius') as number}px</span></div><input type="range" min="0" max="30" value={getCurrentValue('radius', 'radius') as number} onChange={(e) => updateValue('radius', 'radius', parseInt(e.target.value))} className="w-full accent-blue-500 h-1.5 bg-zinc-700 rounded-lg appearance-none cursor-pointer"/></div>
                </div>
