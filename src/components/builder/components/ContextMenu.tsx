...
import {
  ...
  Eye, EyeOff, MousePointer2, MousePointer2Off, CheckSquare, Square,
...
const ContextMenu: React.FC<Props> = memo(({
  state, onClose, isSelected, isVisible,
  onBringToFront, onBringForward, onSendBackward, onSendToBack,
  onHide, onShowAll, onSelect, onDeselect, onSelectAll, onDeselectAll,
  onDuplicate, onResetBadge, onDelete,
}) => {
  ...
  const groups: Group[] = [
    {
      items: [
        { id: 'front',   label: 'Bring to Front',   icon: <ArrowUpToLine size={12} />,   shortcut: '⌘⇧]' },
        { id: 'forward', label: 'Bring Forward',     icon: <ArrowUp size={12} />,         shortcut: '⌘]'  },
        { id: 'back',    label: 'Send Backward',     icon: <ArrowDown size={12} />,       shortcut: '⌘['  },
        { id: 'toback',  label: 'Send to Back',      icon: <ArrowDownToLine size={12} />, shortcut: '⌘⇧[' },
      ],
    },
    {
      items: [
        ...
        { id: 'select',   label: isSelected ? 'Deselect Badge' : 'Select Badge',
                                                     icon: isSelected ? <Square size={12} /> : <MousePointer2 size={12} /> },
        { id: 'selectall',   label: 'Select All',    icon: <CheckSquare size={12} /> },
        { id: 'deselectall', label: 'Deselect All',  icon: <MousePointer2Off size={12} />, shortcut: '⌘D' },
      ],
    },
    {
      items: [
  ...
  const handleAction = (actionId: string) => {
    switch (actionId) {
      case 'front':      exec(() => onBringToFront(id)); break;
      case 'forward':    exec(() => onBringForward(id)); break;
      case 'back':       exec(() => onSendBackward(id)); break;
      case 'toback':     exec(() => onSendToBack(id)); break;
      case 'hide':       exec(() => onHide(id)); break;
      case 'showall':    exec(() => onShowAll()); break;
      case 'select':     exec(() => isSelected ? onDeselect(id) : onSelect(id)); break;
      case 'selectall':  exec(() => onSelectAll()); break;
      case 'deselectall':exec(() => onDeselectAll()); break;
      case 'dup':        exec(() => onDuplicate?.(id)); break;
      case 'reset':      exec(() => onResetBadge(id)); break;
      case 'delete':     exec(() => onDelete(id)); break;
    }
  };
          ...
          {group.items.map((item) => (
            <button
              ...
              onMouseLeave={(e) => {
                ...
                el.style.color = item.danger ? 'rgba(248,113,113,0.85)' : 'rgba(240,230,204,0.75)';
              }}
            >
              ...
              {item.shortcut && (
                <span style={{
                  fontSize: 9, color: 'rgba(140,130,112,0.45)',
                  fontFamily: 'JetBrains Mono, monospace',
