// src/components/builder/components/PreviewCanvas.tsx — verbatim
...
const PreviewCanvas: React.FC<Props> = ({ config, setConfig, selectedIds, onSelect }) => {
  ...
  useEffect(() => {
    const resize = () => {
      ...
      const pad = 32;
      setAutoScale(Math.min((containerRef.current.clientWidth  - pad) / CANVAS_WIDTH, (containerRef.current.clientHeight - pad) / CANVAS_HEIGHT, 1));
    };
    resize();
  ...
  const clampPan = (x: number, y: number) => ({ x: Math.max(-CANVAS_WIDTH/3, Math.min(CANVAS_WIDTH/3, x)), y: Math.max(-CANVAS_HEIGHT/3, Math.min(CANVAS_HEIGHT/3, y)) });
  ...
  const handleWheel = (e: React.WheelEvent) => {
    if (e.ctrlKey || e.metaKey) { e.preventDefault(); setZoomFlash(zoom * Math.exp(-e.deltaY * 0.004)); }
    else {
      let dx = e.deltaX, dy = e.deltaY;
      if (e.shiftKey) { dx = e.deltaY || e.deltaX; dy = 0; }
      setIsPanning(true); setPan(p => clampPan(p.x - dx * 0.85, p.y - dy * 0.85));
  ...
  }, [config.tmdbId, config.source, config.mediaType, config.textless, config.ptype, config.posterBlur, config.grayscale, config.keys?.tmdb, config.keys?.fanart]);

  useEffect(() => {
    if (!cleanPosterUrl) { setIsImageLoading(false); setImageError(false); return; }
  ...
  const handleLogoDragEnd = useCallback((dx: number, dy: number) => {
    setConfig(prev => {
      ...
      return { ...prev, logoX: Math.round(Math.max(-(prev.logoW * margin), Math.min(currentX + dx, CANVAS_WIDTH  - prev.logoW  * (1 - margin)))), logoY: Math.round(Math.max(-(prev.logoH * margin), Math.min(prev.logoY  + dy, CANVAS_HEIGHT - prev.logoH * (1 - margin)))) };
    });
  }, [setConfig]);
  ...
  const handleDragMove = useCallback((id: RatingType, dx: number, dy: number) => {
    ...
    if (dragRafRef.current === null) {
      dragRafRef.current = requestAnimationFrame(() => { dragRafRef.current = null; if (pendingDragRef.current) setDragSession(pendingDragRef.current); });
    }
  }, []);
  ...
  const handleDragEnd = (id: RatingType, dx: number, dy: number) => {
    ...
    setConfig((prev: PosterConfig) => {
      ...
      const applyDelta = (targetId: RatingType) => {
        if (!newItems[targetId]) { const auto = calculateAutoPosition(targetId, prev.ratings.indexOf(targetId), prev.ratings.length, prev); newItems[targetId] = { x: auto.x, y: auto.y }; }
        let sx = newItems[targetId].x, sy = newItems[targetId].y;
        if (sx === undefined || sy === undefined) { const auto = calculateAutoPosition(targetId, prev.ratings.indexOf(targetId), prev.ratings.length, prev); sx = sx ?? auto.x; sy = sy ?? auto.y; }
        const s = getScale(prev.size) * (newItems[targetId]?.scale ?? 1.0);
        const bW = BASE_BADGE_W * s, bH = BASE_BADGE_H * s, offX = bW * 0.4, offY = bH * 0.4;
  ...
  const checkOverlap = (id1: RatingType, idx1: number, id2: RatingType, idx2: number) => {
    const getRect = (id: RatingType, idx: number) => {
      const cfg = config.items[id], auto = calculateAutoPosition(id, idx, config.ratings.length, config);
      const x = cfg?.x ?? auto.x, y = cfg?.y ?? auto.y, s = getScale(config.size) * (cfg?.scale ?? 1.0);
  ...
  return (
    <div ref={containerRef} className="w-full h-full flex items-center justify-center relative overflow-hidden bg-[#18181b] touch-none select-none"
      ...
      <div className="absolute right-3 flex flex-col items-center gap-1.5 z-30 transition-all duration-300"
        ...
        <div className="bg-black/80 backdrop-blur-sm border border-white/10 rounded-full px-2 py-0.5 text-[10px] font-mono text-zinc-300 pointer-events-none transition-opacity duration-300" style={{ opacity: isZooming ? 1 : 0 }}>{Math.round(zoom * 100)}%</div>
      ...
      <div style={{ width: CANVAS_WIDTH, height: CANVAS_HEIGHT, transform: `translate(${pan.x}px, ${pan.y}px) scale(${currentScale})`, transition: isPanning ? 'none' : 'transform 0.18s cubic-bezier(0,0,0.2,1)' }}
        ...
        {isImageLoading && cleanPosterUrl && (
          <div className="absolute inset-0 z-40 flex items-center justify-center bg-zinc-900/60 backdrop-blur-sm pointer-events-none">
            <div className="flex flex-col items-center gap-3"><Loader2 className="animate-spin text-[#C47C2E]" size={36} strokeWidth={2} /><span className="text-[10px] text-zinc-500 font-mono">loading poster…</span></div>
          </div>
        )}
        {(imageError || !cleanPosterUrl) && !isImageLoading && (
          <div className="absolute inset-0 z-40 flex flex-col items-center justify-center gap-2 bg-zinc-900/70 pointer-events-none">
            ...
            <span className="text-[11px] text-zinc-600 font-mono">{!cleanPosterUrl ? 'search for a title to preview' : 'failed to load poster'}</span>
          </div>
        )}
        ...
        {cleanPosterUrl && (
          <img key={cleanPosterUrl} src={cleanPosterUrl} alt="Poster preview"
        ...
        {config.ratings.map((id: RatingType, index: number) => {
          ...
          if (!isFinite(x)) x = auto.x; if (!isFinite(y)) y = auto.y;
          let isObscuring = false;
          if (hoveredBadgeId && hoveredBadgeId !== id) isObscuring = checkOverlap(id, index, hoveredBadgeId, config.ratings.indexOf(hoveredBadgeId));
          if (dragSession) {
            const isTarget = dragSession.id === id, isGroup = selectedIds.has(dragSession.id) && selectedIds.has(id);
            if (isTarget || isGroup) {
        ...
        {config.logo && <DraggableLogo config={config} logoUrl={logoPreviewUrl} canvasScale={currentScale} onDragEnd={handleLogoDragEnd} />}
      </div>
    </div>
  );
};
