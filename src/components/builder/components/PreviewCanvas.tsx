...
const PreviewCanvas: React.FC<Props> = ({ config, setConfig, selectedIds, onSelect }) => {
  ...
  // ── Badge rect for overlap detection ─────────────────────────────────────
  // Uses the same scale formula as DraggableBadge: globalScale * itemScale
  const getBadgeRect = (id: RatingType, index: number) => {
    ...
    // Match DraggableBadge: globalScale (includes size tier AND config.scale) × item.scale
    ...
    const w           = BASE_BADGE_W * badgeScale;
    const h           = BASE_BADGE_H * badgeScale;
    return { x, y, w, h };
  };
  ...
  const handleLogoDragEnd = (dx: number, dy: number) => {
    setConfig((prev) => {
      ...
      const newY     = Math.round(Math.max(-(prev.logoH * margin), Math.min(currentY + dy, CANVAS_HEIGHT - prev.logoH * (1 - margin))));
  ...
  const handleDragEnd = (id: RatingType, dx: number, dy: number) => {
    ...
    setConfig((prev: PosterConfig) => {
      ...
      const applyDelta = (targetId: RatingType) => {
        ...
        // Clamp with generous margin so badges can be partially off-canvas (30%)
        const globalScale = getScale(prev.size) * (prev.scale !== undefined ? prev.scale : 1.0);
        const badgeScale  = globalScale * (newItems[targetId]?.scale ?? 1.0);
        const selWidth    = BASE_BADGE_W * badgeScale;
        const selHeight   = BASE_BADGE_H * badgeScale;
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col items-center gap-1 bg-zinc-900/90 backdrop-blur border border-white/10 rounded-full p-1.5 shadow-xl">
          <button
            onClick={() => { const next = Math.min(zoom + 0.15, 4); setZoom(next); setIsZooming(true); if (zoomFadeTimer.current) clearTimeout(zoomFadeTimer.current); zoomFadeTimer.current = setTimeout(() => setIsZooming(false), 800); }}
            className="p-2 text-zinc-400 hover:text-[#D4A245] rounded-full hover:bg-[#C47C2E]/10 active:scale-95 transition-all"
          >
            <ZoomIn size={18} />
          </button>
          <button
            onClick={() => { const next = Math.max(zoom - 0.15, 0.2); setZoom(next); setIsZooming(true); if (zoomFadeTimer.current) clearTimeout(zoomFadeTimer.current); zoomFadeTimer.current = setTimeout(() => setIsZooming(false), 800); }}
            className="p-2 text-zinc-400 hover:text-[#D4A245] rounded-full hover:bg-[#C47C2E]/10 active:scale-95 transition-all"
          >
            <ZoomOut size={18} />
          </button>
        ...
        {config.ratings.map((id: RatingType, index: number) => {
          ...
          if (dragSession) {
            ...
            if (isTarget || isGroup) {
              const globalScale = getScale(config.size) * (config.scale !== undefined ? config.scale : 1.0);
              const badgeScale  = globalScale * (itemConfig?.scale ?? 1.0);
              const bW     = BASE_BADGE_W * badgeScale;
              const bH     = BASE_BADGE_H * badgeScale;
