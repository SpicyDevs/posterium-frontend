  ...
  return (
    <Transition appear show={isOpen} as={Fragment}>
      <Dialog as="div" className="relative z-50" onClose={onClose}>
        ...
        >
          <div className="fixed inset-0" style={{ background: 'rgba(7,7,6,0.75)', backdropFilter: 'blur(12px)' }} />
        </TransitionChild>
          ...
          >
            ...
            >
              ...
              <div className="flex items-center justify-between px-5 py-4"
                style={{ borderBottom: '1px solid rgba(255,255,255,0.06)', background: 'rgba(196,124,46,0.02)' }}>
                <DialogTitle as="div" className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center"
                    style={{ background: 'rgba(196,124,46,0.12)', border: '1px solid rgba(196,124,46,0.2)' }}>
                    <Keyboard size={14} style={{ color: 'var(--film-amber)' }} />
                  </div>
                  <div>
                    <p className="text-[13px] font-semibold" style={{ color: 'var(--film-cream)', fontFamily: 'Syne, sans-serif' }}>
                      Keyboard Shortcuts
                    </p>
                    <p className="text-[9px]" style={{ color: 'rgba(140,130,112,0.5)', fontFamily: 'JetBrains Mono, monospace', letterSpacing: '0.1em' }}>
                ...
                <button onClick={onClose}
                  ...
                  style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', color: 'rgba(140,130,112,0.6)', cursor: 'pointer' }}
              ...
              <div className="grid grid-cols-2 gap-0 max-h-[70vh] overflow-y-auto"
                style={{ scrollbarWidth: 'thin', scrollbarColor: 'rgba(196,124,46,0.2) transparent' }}>
                {SHORTCUT_GROUPS.map((group, gi) => (
                  <div key={group.title}
                    className="p-4"
                    style={{ borderBottom: gi < SHORTCUT_GROUPS.length - 2 ? '1px solid rgba(255,255,255,0.04)' : 'none', borderRight: gi % 2 === 0 ? '1px solid rgba(255,255,255,0.04)' : 'none' }}>
                    ...
                    <div className="space-y-2.5">
                      {group.shortcuts.map(({ keys, label }) => (
                        <div key={label} className="flex items-center justify-between gap-3">
                          <span className="text-[11px]" style={{ color: 'rgba(240,230,204,0.7)', fontFamily: 'DM Sans, sans-serif' }}>
                            {label}
                          </span>
                          <div className="flex items-center gap-1 shrink-0">
                            {keys.map((k, i) => (
                              <React.Fragment key={k}>
                                {i > 0 && <span style={{ fontSize: 9, color: 'rgba(122,117,110,0.3)' }}>+</span>}
                                <Kbd>{k}</Kbd>
                              </React.Fragment>
                            ))}
                          </div>
              ...
              <div className="flex items-center justify-between px-5 py-3"
                ...
                <span style={{ fontSize: 9, fontFamily: 'JetBrains Mono, monospace', color: 'rgba(140,130,112,0.3)', letterSpacing: '0.1em' }}>
                  Press <Kbd>Esc</Kbd> or <Kbd>⌘/</Kbd> to close
                </span>
                <span style={{ fontSize: 9, fontFamily: 'JetBrains Mono, monospace', color: 'rgba(140,130,112,0.2)', letterSpacing: '0.08em' }}>
