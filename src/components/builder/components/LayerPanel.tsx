// src/components/builder/components/LayerPanel.tsx — verbatim
...
const InlineSlider: React.FC<{ label: string; value: number; onChange: (v: number) => void; min: number; max: number; step?: number; unit?: string; formatValue?: (v: number) => string }> = ({ label, value, onChange, min, max, step = 1, unit = "", formatValue }) => {
  const display = formatValue ? formatValue(value) : `${value}${unit}`;
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between"><span className="text-[10px] text-zinc-500 font-medium">{label}</span><span className="text-[10px] font-mono text-zinc-600 tabular-nums">{display}</span></div>
      <input type="range" min={min} max={max} step={step} value={value} onChange={e => onChange(parseFloat(e.target.value))} className="w-full" />
    </div>
  );
};

const LOGO_SIZES = [{ key: "sm", label: "S", w: 220, h: 58 }, { key: "md", label: "M", w: 320, h: 84 }, { key: "lg", label: "L", w: 400, h: 105 }, { key: "xl", label: "XL", w: 460, h: 121 }] as const;
function getLogoSizeKey(w: number, h: number) { for (const s of LOGO_SIZES) { if (s.w === w && s.h === h) return s.key; } return "custom"; }
const LOGO_SOURCES: { id: LogoSourceType; label: string }[] = [{ id: null, label: "Auto" }, { id: "fanart", label: "Fanart" }, { id: "tmdb", label: "TMDB" }, { id: "metahub", label: "Hub" }];
...
const LogoPanel: React.FC<{ config: PosterConfig; setConfig: React.Dispatch<React.SetStateAction<PosterConfig>> }> = ({ config, setConfig }) => {
  ...
  const update = useCallback(<K extends keyof PosterConfig>(key: K, value: PosterConfig[K]) => setConfig(prev => ({ ...prev, [key]: value })), [setConfig]);
    ...
    <div className="space-y-4 pt-1">
      <div className="space-y-1.5">
        ...
        <div className="grid grid-cols-4 gap-1">
          {LOGO_SOURCES.map(opt => (
            <button key={String(opt.id)} type="button" onClick={() => update("logoSource", opt.id as PosterConfig["logoSource"])}
              className={clsx("h-9 rounded-lg text-[11px] font-medium transition-all active:scale-95", (config.logoSource ?? null) === opt.id ? "bg-[#C47C2E]/15 text-[#E8D8A8] ring-1 ring-[#C47C2E]/30" : "bg-[#111113] text-zinc-400 hover:text-zinc-200 border border-white/6 hover:border-white/15")}>{opt.label}</button>
          ))}
        </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <button type="button" onClick={() => setShowCustomSize(v => !v)} className="text-[9px] text-zinc-600 hover:text-zinc-400 transition-colors">{showCustomSize || currentSizeKey === "custom" ? "Presets" : "Custom"}</button>
        </div>
        <div className="grid grid-cols-4 gap-1">
          {LOGO_SIZES.map(sz => (
            <button key={sz.key} type="button" onClick={() => { setConfig(p => ({ ...p, logoW: sz.w, logoH: sz.h })); setShowCustomSize(false); }}
              className={clsx("flex flex-col items-center justify-center h-12 rounded-lg transition-all active:scale-95 border", currentSizeKey === sz.key && !showCustomSize ? "bg-[#C47C2E]/15 text-[#E8D8A8] border-[#C47C2E]/30 ring-1 ring-[#C47C2E]/20" : "bg-[#111113] text-zinc-400 hover:text-zinc-200 border-white/6 hover:border-white/15")}>
        ...
        {(showCustomSize || currentSizeKey === "custom") && (
          <div className="space-y-3 pt-2 pl-2 border-l-2 border-white/5">
            <InlineSlider label="Width"  value={config.logoW} min={50} max={490} unit="px" onChange={v => setConfig(p => ({ ...p, logoW: Math.round(v) }))} />
            <InlineSlider label="Height" value={config.logoH} min={20} max={200} unit="px" onChange={v => update("logoH", Math.round(v))} />
          </div>
        )}
      </div>
      <div className="space-y-3">
        <p className="text-[10px] font-semibold text-zinc-500 uppercase tracking-widest">Position</p>
        <InlineSlider label="Vertical position" value={config.logoY} min={0} max={748} unit="px" onChange={v => update("logoY", Math.round(v))} />
        <div className="flex items-center justify-between gap-3 px-0.5">
          <div><p className="text-[10px] text-zinc-400 font-medium">Auto-centre horizontally</p><p className="text-[9px] text-zinc-600">Centre at {Math.round((CANVAS_WIDTH - config.logoW) / 2)} px</p></div>
          <Switch checked={config.logoX === null} onChange={v => update("logoX", v ? null : Math.round((CANVAS_WIDTH - config.logoW) / 2))} className={clsx("relative inline-flex h-5 w-9 shrink-0 items-center rounded-full transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[#C47C2E]", config.logoX === null ? "bg-[#C47C2E]" : "bg-zinc-700")}>
      ...
      <div className="space-y-3">
        ...
        <InlineSlider label="Opacity" value={config.logoOpacity} min={0} max={1} step={0.05} formatValue={v => `${Math.round(v * 100)}%`} onChange={v => update("logoOpacity", v)} />
        <InlineSlider label="Drop Shadow" value={config.logoShadow} min={0} max={30} onChange={v => update("logoShadow", v)} />
      </div>
      <div className="flex items-start gap-2 px-2.5 py-2 rounded-lg bg-white/[0.02] border border-white/[0.04]">
        ...
        <p className="text-[9px] text-zinc-600 leading-relaxed">Drag the logo directly on the canvas to reposition. Snap guides appear near the centre.</p>
      </div>
    </div>
  );
};
...
const LayerPanel: React.FC<Props> = ({ config, setConfig, selectedIds, onSelect }) => {
  ...
  useEffect(() => {
    const t = setTimeout(async () => {
      ...
      setIsSearching(true);
      try { const res = await fetch(`https://api.spicydevs.xyz/search?q=${encodeURIComponent(searchQuery)}`); const data = await res.json(); if (data.results) setResults(data.results.filter((i: SearchResult) => i.poster_path && ["movie","tv"].includes(i.media_type))); } catch { } finally { setIsSearching(false); }
    }, 400);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${DEFAULT_API_BASE}/${config.mediaType}/${config.tmdbId}.json?source=${config.source}`, { signal: ctrl.signal });
        if (!res.ok) return;
        ...
        const posterPath = data.poster?.selected || data.meta?.poster || "";
  ...
  const updateConfig = useCallback(<K extends keyof PosterConfig>(key: K, value: PosterConfig[K]) => setConfig(prev => ({ ...prev, [key]: value })), [setConfig]);

  const handleToggleVisibility = useCallback((id: RatingType, visible: boolean) => {
    if (visible) { setConfig(prev => prev.ratings.includes(id) ? prev : { ...prev, ratings: [id, ...prev.ratings] }); setInactiveOrder(prev => prev.filter(x => x !== id)); }
    else { setConfig(prev => ({ ...prev, ratings: prev.ratings.filter(r => r !== id) })); setInactiveOrder(prev => [id, ...prev.filter(x => x !== id)]); }
  }, [setConfig]);

  const allVisible = config.ratings.length === ALL_BADGES.length;
  const handleToggleAll = useCallback(() => {
    if (allVisible) { setConfig(prev => { setInactiveOrder(io => [...[...prev.ratings].reverse(), ...io]); return { ...prev, ratings: [] }; }); }
    else { setConfig(prev => { const missing = ALL_BADGES.filter(b => !prev.ratings.includes(b.id)).map(b => b.id); return { ...prev, ratings: [...missing, ...prev.ratings] }; }); setInactiveOrder([]); }
  }, [allVisible, setConfig]);

  const allVisibleSelected = config.ratings.length > 0 && config.ratings.every(r => selectedIds.has(r));
  const handleSelectAll = useCallback((checked: boolean) => { setBatchSelection(checked ? ALL_BADGES.filter(b => config.ratings.includes(b.id)).map(b => b.id) : []); }, [setBatchSelection, config.ratings]);
  ...
  const activeBadges = [...config.ratings].reverse().map(id => ALL_BADGES.find(b => b.id === id)).filter((b): b is { id: RatingType; label: string } => !!b);
  ...
  const getIconKey = (id: string): BadgeIconKey => id === "rt" ? "rt_fresh" : id === "rt_popcorn" ? "popcorn_fresh" : (id as BadgeIconKey);
  const MediaIcon = config.mediaType === "tv" ? Tv : config.mediaType === "anime" ? Clapperboard : Film;

  const sourceOptions = [
    { id: "tmdb", label: "TMDB" }, { id: "fanart", label: "Fanart.tv" }, { id: "metahub", label: "Metahub" }, { id: "imdb", label: "IMDb" },
    ...(config.mediaType === "anime" ? [{ id: "mal", label: "MyAnimeList" }, { id: "anilist", label: "AniList" }] : []),
  ];
  const ptypeOptions = [
    { id: "auto", label: "Auto (Default)" }, { id: "top1", label: "Top 1" }, { id: "top2", label: "Top 2" }, { id: "top3", label: "Top 3" },
  ...
  const renderBadgeRow = (badge: { id: RatingType; label: string }, isActive: boolean, provided?: DraggableProvided, isDraggingItem?: boolean) => {
    ...
    return (
      <div ref={provided?.innerRef} {...provided?.draggableProps} style={provided?.draggableProps.style}
        ...
        className={clsx("flex items-center gap-2 px-2 py-2 rounded-lg transition-all select-none", isSel ? "bg-[#C47C2E]/12 ring-1 ring-[#C47C2E]/30" : isActive ? "hover:bg-white/4 cursor-pointer" : "opacity-50", isDraggingItem && "shadow-2xl bg-[#1c1c1f] ring-1 ring-white/10 rotate-[0.5deg]")}>
        {isActive ? (
        ...
        <div className="w-7 h-7 shrink-0 rounded-md bg-[#111113] border border-white/6 flex items-center justify-center">
          {badge.id === "age" ? <span className="text-[8px] font-bold" style={{ color: iconColor }}>PG</span> :
           iconData ? <svg viewBox={iconData.vb} className="w-3.5 h-3.5" style={{ color: iconColor }} dangerouslySetInnerHTML={{ __html: iconData.body }} /> :
        ...
        <div className="flex bg-[#111113] rounded-lg p-0.5 border border-white/6">
          {(["source", "layers"] as const).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={clsx("flex-1 flex items-center justify-center gap-1.5 py-2 rounded-md text-[11px] font-medium transition-all duration-150 outline-none select-none capitalize", localMode === tab ? "bg-[#1c1c1f] text-zinc-100 shadow-sm" : "text-zinc-500 hover:text-zinc-300")}>
      ...
      {localMode === "source" && (
        <div className="space-y-4">
          {(fetchedData.title || config.tmdbId) && (
            <div className="flex gap-3 p-2.5 rounded-xl bg-[#111113] border border-white/6">
              <div className="w-12 h-[4.5rem] shrink-0 rounded-lg overflow-hidden bg-zinc-900 border border-white/6">
                {posterUrl ? <img src={posterUrl.startsWith("http") ? posterUrl : `https://image.tmdb.org/t/p/w185${posterUrl}`} alt={fetchedData.title || "Poster"} className="w-full h-full object-cover" loading="lazy" /> : <div className="w-full h-full flex items-center justify-center"><Film size={16} className="text-zinc-600" /></div>}
              </div>
              <div className="flex-1 min-w-0 py-0.5">
                <p className="text-[12px] font-semibold text-zinc-100 leading-tight line-clamp-2">{fetchedData.title || <span className="text-zinc-500 italic">Untitled</span>}</p>
                <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                  {fetchedData.year && <span className="text-[10px] text-zinc-500 font-mono">{fetchedData.year}</span>}
                  <span className={clsx("inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-semibold uppercase tracking-wide", config.mediaType === "tv" ? "bg-blue-500/15 text-blue-400" : config.mediaType === "anime" ? "bg-purple-500/15 text-purple-400" : "bg-amber-500/15 text-amber-400")}>
          ...
          <div className="grid grid-cols-[1fr_auto] gap-2">
            <div><label className="sidebar-label">Media Type</label><SelectBox value={config.mediaType} onChange={v => updateConfig("mediaType", v as PosterConfig["mediaType"])} options={[{ id: "movie", label: "🎬 Movie" }, { id: "tv", label: "📺 TV Series" }, { id: "anime", label: "🎌 Anime" }]} /></div>
            <div className="w-24"><label className="sidebar-label">ID</label><input type="text" value={config.tmdbId} onChange={e => updateConfig("tmdbId", e.target.value)} className="w-full h-9 px-2 rounded-lg bg-[#111113] border border-white/[0.08] text-[11px] font-mono text-zinc-300 text-center focus:outline-none focus-visible:border-[#C47C2E]/50 transition-colors" /></div>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div><label className="sidebar-label">Poster Source</label><SelectBox value={config.source} onChange={v => updateConfig("source", v as PosterConfig["source"])} options={sourceOptions} /></div>
            {["fanart","tmdb","imdb"].includes(config.source) && <div><label className="sidebar-label">Poster Type</label><SelectBox value={config.ptype || "auto"} onChange={v => updateConfig("ptype", v)} options={ptypeOptions} /></div>}
          </div>
          <div className={clsx("flex items-center justify-between px-3 py-3 rounded-xl bg-[#111113] border border-white/6 transition-opacity", ["metahub","imdb"].includes(config.source) && "opacity-50 pointer-events-none")}>
            <div><p className="text-[11px] font-medium text-zinc-300">Textless Poster</p><p className="text-[9px] text-zinc-500 mt-0.5">Remove title text from image</p></div>
            <Switch checked={["metahub","imdb"].includes(config.source) ? false : config.textless} onChange={v => updateConfig("textless", v)} disabled={["metahub","imdb"].includes(config.source)}
              className={clsx("relative inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[#C47C2E]", config.textless && !["metahub","imdb"].includes(config.source) ? "bg-[#C47C2E]" : "bg-zinc-700")}>
          ...
          <div className="rounded-xl border border-white/6 overflow-hidden">
            <div className="flex items-center justify-between px-3 py-3 bg-[#111113]">
              <div className="flex items-center gap-2.5">
                <div className={clsx("w-7 h-7 rounded-lg flex items-center justify-center transition-colors", config.logo ? "bg-[#C47C2E]/20 text-[#D4A245]" : "bg-white/4 text-zinc-600")}><ImagePlay size={14} /></div>
                <div><p className="text-[11px] font-semibold text-zinc-200 leading-tight">Logo Overlay</p><p className="text-[9px] text-zinc-600">{config.logo ? `${config.logoSource ?? "Auto"} · ${config.logoW}×${config.logoH}` : "Transparent title art overlay"}</p></div>
              </div>
              <Switch checked={config.logo} onChange={v => updateConfig("logo", v)} className={clsx("relative inline-flex h-5 w-9 shrink-0 items-center rounded-full transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[#C47C2E]", config.logo ? "bg-[#C47C2E]" : "bg-zinc-700")}>
                <span className={clsx("inline-block w-3.5 h-3.5 rounded-full bg-white shadow-sm transition-transform", config.logo ? "translate-x-[18px]" : "translate-x-[3px]")} />
              </Switch>
            </div>
            {config.logo && <div className="px-3 pb-4 bg-[#0f0f11] border-t border-white/[0.05]"><LogoPanel config={config} setConfig={setConfig} /></div>}
          </div>
        </div>
      )}
      {localMode === "layers" && (
        <div>
          <div className="flex items-center justify-between mb-3 px-1">
            ...
            <div className="flex items-center gap-2">
              <button onClick={handleToggleAll} className="flex items-center gap-1.5 text-[10px] text-zinc-500 hover:text-zinc-200 transition-colors" title={allVisible ? "Hide all" : "Show all"}>{allVisible ? <Eye size={11} /> : <EyeOff size={11} />}{allVisible ? "Hide all" : "Show all"}</button>
              ...
              <button onClick={() => handleSelectAll(!allVisibleSelected)} className="flex items-center gap-1.5 text-[10px] text-zinc-500 hover:text-zinc-200 transition-colors">
                <div className={clsx("w-3.5 h-3.5 rounded border flex items-center justify-center transition-all", allVisibleSelected ? "bg-[#C47C2E] border-[#D4A245]" : "border-zinc-600")}>{allVisibleSelected && <Check size={9} className="text-white" />}</div>
                Select all
              </button>
            </div>
          </div>
          {activeBadges.length > 0 ? (
            ...
            <div className="flex flex-col items-center py-10 text-zinc-600 gap-2"><EyeOff size={22} strokeWidth={1.5} /><p className="text-[11px] text-zinc-500">No active badges</p><p className="text-[9px] text-zinc-700">Enable some from the list below</p></div>
          )}
          {inactiveBadges.length > 0 && (
            <><div className="mt-5 mb-2 px-1"><span className="sidebar-label mb-0">Available</span></div>
            <div className="space-y-0.5">{inactiveBadges.map(badge => <React.Fragment key={badge.id}>{renderBadgeRow(badge, false)}</React.Fragment>)}</div></>
          )}
        </div>
      )}
    </SidebarLayout>
