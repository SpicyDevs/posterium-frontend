...
/**
 * LogoPanel — Source + Size + Appearance controls.
 * Position is handled by dragging the logo directly on the canvas.
 */
    ...
    <div className="space-y-4 pt-1">
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <button type="button" onClick={() => setShowCustomSize(v => !v)}
        ...
        <div className="grid grid-cols-4 gap-1">
          {LOGO_SIZES.map(sz => (
            <button key={sz.key} type="button"
              ...
              className={clsx("flex flex-col items-center justify-center h-12 rounded-lg transition-all active:scale-95 border",
      ...
      <div className="flex items-start gap-2 px-2.5 py-2 rounded-lg bg-white/[0.02] border border-white/[0.04]">
        ...
        <p className="text-[9px] text-zinc-600 leading-relaxed">
          Drag the logo directly on the canvas to reposition it. Snap guides appear near the centre.
        </p>
      </div>
    </div>
  );
...
const LayerPanel: React.FC<Props> = ({ config, setConfig, selectedIds, onSelect }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // ── Resolve direct poster URL for PreviewCanvas background ──────────
        // Use the "selected" poster URL from the JSON response — this is the
        // direct TMDB/source image URL, NOT the SVG API wrapper.
        const rawPoster = data.poster?.selected || data.meta?.poster || "";
        if (rawPoster) {
          const fullPosterUrl = rawPoster.startsWith("http")
            ? rawPoster
            : `https://image.tmdb.org/t/p/w500${rawPoster}`;
          setPosterUrl(fullPosterUrl);
          // Share with PreviewCanvas via context so it renders the direct image
          setLivePosterUrl(fullPosterUrl);
        }

        // ── Store resolved imdbId so generateApiUrl can use /poster/ path ───
    ...
    return () => ctrl.abort();
  }, [config.tmdbId, config.mediaType, config.source, setLiveRatings, setLivePosterUrl, setConfig]);

  // Clear poster when media changes
  useEffect(() => {
    if (!config.tmdbId) {
      setPosterUrl("");
  ...
  const handleSelectMedia = useCallback((item: SearchResult | null) => {
    ...
    // Use w500 for the preview (good quality, reasonable size)
    const previewUrl = item.poster_path ? `https://image.tmdb.org/t/p/w500${item.poster_path}` : "";
    setPosterUrl(previewUrl);
    setLivePosterUrl(previewUrl || null);
    setConfig(prev => ({
      ...prev,
      tmdbId: item.id.toString(),
      imdbId: undefined, // will be resolved once JSON data loads
  ...
  const renderBadgeRow = (
    ...
    return (
      ...
      >
        {isActive ? (
          <div {...provided?.dragHandleProps} onClick={e => e.stopPropagation()} className="text-zinc-700 hover:text-zinc-400 cursor-grab active:cursor-grabbing p-0.5 outline-none transition-colors shrink-0">
        ...
        <div className="space-y-4">
                    ...
                    <Combobox.Options className="absolute top-full mt-1 z-50 w-full bg-[#1c1c1f] border border-white/10 rounded-xl shadow-2xl shadow-black/50 max-h-64 overflow-y-auto custom-scrollbar py-1.5 focus:outline-none">
                      {results.map(item => (
                        <Combobox.Option key={item.id} value={item}
                          className={({ active }) => clsx("flex items-center gap-3 px-3 py-2.5 cursor-pointer transition-colors", active && "bg-[#C47C2E]/12")}>
                          <img
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-1.5 mt-0.5">
                              ...
                              <span className={clsx("text-[9px] px-1 py-px rounded font-semibold uppercase",
          ...
          <div className="grid grid-cols-[1fr_auto] gap-2">
            ...
            <div className="w-24">
              ...
              <input type="text" value={config.tmdbId}
        ...
        <div>
          ...
          <div className="flex items-center justify-between mb-3 px-1">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => handleSelectAll(!allVisibleSelected)}
                className="flex items-center gap-1.5 text-[10px] text-zinc-500 hover:text-zinc-200 transition-colors">
