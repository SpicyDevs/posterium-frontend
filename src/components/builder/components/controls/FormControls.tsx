...
export const Section: React.FC<{
  ...
  return (
    <div className={compact ? 'pt-5' : 'pt-5 first:pt-3'}>
      <button
        type="button"
        onClick={toggle}
        className={clsx(
      ...
      {open && (
        <div className={clsx('pb-1 space-y-3.5', compact ? 'px-1' : 'px-3')}>{children}</div>
      )}
      <div
        className={clsx('mt-5', compact ? 'mx-1' : 'mx-3')}
...
export const SegmentedRow: React.FC<{
  ...
  heading?: boolean;
}> = ({ label, options, value, onChange, heading = false }) => (
  <div className="space-y-1.5">
    {heading ? (
      <p
    ...
    <div className="grid gap-1" style={{ gridTemplateColumns: `repeat(${options.length}, 1fr)` }}>
      {options.map((opt) => (
        <button
          ...
          className={clsx(
            heading ? 'h-8' : 'h-7',
            'rounded-md text-[10px] font-medium transition-all border syne-font',
            value === opt.id
