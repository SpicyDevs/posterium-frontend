...
const ShortcutsModal: React.FC<Props> = memo(({ isOpen, onClose }) => (
  <Dialog.Root open={isOpen} onOpenChange={open => !open && onClose()}>
    <Dialog.Portal>
      ...
      />
      <Dialog.Content
        className="fixed left-1/2 top-1/2 z-50 w-full max-w-2xl -translate-x-1/2 -translate-y-1/2 rounded-2xl overflow-hidden shadow-2xl data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95"
        style={{
          ...
          <Dialog.Title asChild>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p
                  className="text-[9px]"
                  style={{ color: 'rgba(140,130,112,0.5)', fontFamily: 'JetBrains Mono, monospace', letterSpacing: '0.1em' }}
                >
              ...
              <div className="space-y-2.5">
                {group.shortcuts.map(({ keys, label }) => (
                  <div key={label} className="flex items-center justify-between gap-3">
                    <span
                      className="text-[11px]"
                      style={{ color: 'rgba(240,230,204,0.7)', fontFamily: 'DM Sans, sans-serif' }}
                    >
                      {label}
                    </span>
