...
// import.meta.env.VITE_API_URL works unchanged in Astro/Vite
...
export const generateApiUrl = (config: PosterConfig, baseUrl: string = DEFAULT_API_BASE): string => {
  ...
  if (config.logo) {
    ...
    if (config.logoX !== null && config.logoX !== undefined) params.set('logo_x', config.logoX.toString());
...
export const parseUrlToConfig = (urlString: string): PosterConfig => {
  try {
    ...
    const extension: ExtensionType = match && match[3] ? ((match[3] === 'jpeg' ? 'jpg' : match[3]) as ExtensionType) : 'svg';
    const params = url.searchParams;
    const keys: ApiKeys = {};
    ...
    ratingKeys.forEach((key) => {
      const x = params.get(`${key}_x`), y = params.get(`${key}_y`);
      const bg = params.get(`${key}_bg`), txt = params.get(`${key}_txt`);
      const blur = params.get(`${key}_blur`), alpha = params.get(`${key}_alpha`);
      const rad = params.get(`${key}_rad`), sh = params.get(`${key}_sh`);
      const icon = params.get(`${key}_icon`), scale = params.get(`${key}_scale`);
      const bw = params.get(`${key}_bw`), bc = params.get(`${key}_bc`);
      if (x || y || bg || txt || blur || alpha || rad || sh || icon || scale || bw) {
        items[key] = {
          ...(x ? { x: parseInt(x) } : {}), ...(y ? { y: parseInt(y) } : {}),
          ...(bg ? { bg } : {}), ...(txt ? { txt: txt.startsWith('#') ? txt : `#${txt}` } : {}),
          ...(blur ? { blur: parseInt(blur) } : {}), ...(alpha ? { alpha: parseFloat(alpha) } : {}),
          ...(rad ? { radius: parseInt(rad) } : {}), ...(sh ? { shadow: parseInt(sh) } : {}),
          ...(icon ? { icon: icon === '1' } : {}), ...(scale ? { scale: parseFloat(scale) } : {}),
          ...(bw ? { borderW: parseInt(bw) } : {}), ...(bc ? { borderC: bc.startsWith('#') ? bc : `#${bc}` } : {}),
        };
      }
    });
    const rawLogoSource = params.get('logo_source');
    const VALID_LOGO_SOURCES: LogoSourceType[] = ['fanart', 'tmdb', 'metahub'];
    const logoSource: LogoSourceType = VALID_LOGO_SOURCES.includes(rawLogoSource as LogoSourceType) ? (rawLogoSource as LogoSourceType) : null;
    return {
      ...
      textless: params.get('textless') === '1', theme: 'glass', size: 'md',
      ...
      borderC: params.get('g_bc')     ? (params.get('g_bc')!.startsWith('#') ? params.get('g_bc')! : `#${params.get('g_bc')}`) : undefined,
      bg:      params.get('g_bg')     || undefined,
      txt:     params.get('g_txt')    ? (params.get('g_txt')!.startsWith('#') ? params.get('g_txt')! : `#${params.get('g_txt')}`) : undefined,
