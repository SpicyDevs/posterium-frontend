...
const ToolbarBtn = memo<{
  ...
  const tooltip = !disabled && (
    <span className="absolute top-full mt-2 left-1/2 -translate-x-1/2 px-2 py-1 rounded-md text-[10px] font-medium border whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-150 delay-300 pointer-events-none z-[200] shadow-lg syne-font"
      style={{ background: 'var(--film-mid)', color: 'var(--film-cream)', borderColor: 'rgba(255,255,255,0.08)' }}>
      {label}
    </span>
  );
  ...
  if (href) return <a href={href} target="_blank" rel="noreferrer noopener" className={cls} aria-label={label} style={activeStyle} {...hoverEvents}>{children}{tooltip}</a>;
  return <button onClick={onClick} disabled={!!disabled} className={cls} aria-label={label} aria-disabled={disabled} style={activeStyle} {...hoverEvents}>{children}{tooltip}</button>;
});
...
const AdvancedNavSidebar = memo<{
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--film-dark)' }}>
      {/* Strip header */}
      <div style={{
        ...
        <span className="syne-font" style={{
          ...
          textTransform: 'uppercase', color: 'rgba(196,124,46,0.4)',
        }}>
          Panels
        </span>
      </div>
      ...
      {/* Nav list */}
      <nav style={{ flex: 1, overflowY: 'auto', paddingBlock: 4 }} aria-label="Builder panels">
          ...
          return (
            ...
            >
              ...
              <div style={{
                ...
                border: `1px solid ${isActive ? 'rgba(196,124,46,0.28)' : 'rgba(255,255,255,0.06)'}`,
                transition: 'all 0.12s ease',
              }}>
                <Icon size={13} style={{ color: isActive ? 'var(--film-amber)' : 'inherit' }} />
              </div>

              {/* Label + desc */}
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                  <span className="syne-font" style={{ fontSize: 11, fontWeight: 700, lineHeight: 1.2 }}>
                    {label}
                  </span>
                  {badge !== null && (
                    <span style={{
                      display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                      minWidth: 16, height: 16, background: 'var(--film-amber)', color: '#070706',
                      fontSize: 8, fontWeight: 800, borderRadius: 99, padding: '0 4px',
                    }}>
                      {badge}
                    </span>
                  )}
                </div>
                <div className="mono-font" style={{ fontSize: 8, color: 'var(--film-text-ghost)', marginTop: 1 }}>
                  {desc}
                </div>
              </div>
              ...
              {isActive && (
                <div style={{
                  ...
                  background: 'var(--film-amber)', flexShrink: 0,
                  boxShadow: '0 0 5px rgba(196,124,46,0.6)',
                }} />
              )}
            </button>
          );
      ...
      <div style={{ padding: '10px 14px', borderTop: '1px solid rgba(255,255,255,0.04)', flexShrink: 0 }}>
        <div className="mono-font" style={{ fontSize: 7, color: 'var(--film-text-ghost)', letterSpacing: '0.1em' }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--film-dark)' }}>
      ...
      <div style={{
        ...
        <span className="syne-font" style={{
          ...
          textTransform: 'uppercase', color: 'var(--film-amber)',
        }}>
          {panelMeta.label}
        </span>
        <span className="mono-font" style={{ fontSize: 8, color: 'var(--film-text-ghost)', marginLeft: 'auto' }}>
          {panelMeta.desc}
        </span>
      </div>
...
const AdvancedStudioLayout: React.FC<{
  ...
  const paletteCommands: PaletteCommand[] = [
    // Panel switching
    { id: 'panel-source',    label: 'Open Source Panel',    category: 'Panels', icon: <Film size={13} />,          action: () => setActiveTab('source') },
    { id: 'panel-layers',    label: 'Open Layers Panel',    category: 'Panels', icon: <Layers size={13} />,        action: () => setActiveTab('layers') },
    { id: 'panel-canvas',   label: 'Open Canvas Panel',    category: 'Panels', icon: <Monitor size={13} />,       action: () => setActiveTab('poster') },
    { id: 'panel-badges',    label: 'Open Badges Panel',    category: 'Panels', icon: <Sliders size={13} />,       action: () => setActiveTab('badges') },
    { id: 'panel-selection', label: 'Open Selection Panel', category: 'Panels', icon: <MousePointer2 size={13} />, action: () => setActiveTab('selection') },
    // View
    { id: 'zoom-fit',   label: 'Zoom to Fit',          category: 'View & Canvas', icon: <Maximize2 size={13} />, shortcut: '⌘1', action: dispatchResetView },
    ...
    { id: 'fullscreen', label: isFullscreen ? 'Exit Fullscreen' : 'Enter Fullscreen', category: 'View & Canvas', icon: isFullscreen ? <Minimize2 size={13} /> : <Maximize2 size={13} />, shortcut: 'F', action: toggleFullscreen },
    ...
    { id: 'safe-area',  label: `${viewOptions.showSafeArea ? 'Hide' : 'Show'} Safe Area`, category: 'View & Canvas', icon: <ShieldCheck size={13} />, action: () => toggleViewOption('showSafeArea') },
    { id: 'right-panel', label: `${rightVisible ? 'Hide' : 'Show'} Right Panel`, category: 'View & Canvas', icon: <PanelRight size={13} />, shortcut: ']', action: () => setRightVisible(v => !v) },
    // Selection
    ...
    { id: 'hide-sel',     label: 'Hide Selected',     category: 'Layers & Selection', icon: <EyeOff size={13} />,          shortcut: 'H', action: () => Array.from(selectedIds).forEach(id => hideBadge(id as RatingType)) },
    // Canvas
    { id: 'grayscale', label: `${config.grayscale ? 'Remove' : 'Apply'} Grayscale`, category: 'Canvas Properties', icon: <Contrast size={13} />, action: () => setConfig(p => ({ ...p, grayscale: !p.grayscale })) },
    // Export
    ...
    { id: 'reset', label: 'Reset All Settings', category: 'File', icon: <RotateCcw size={13} />,               action: () => setIsResetOpen(true) },
    // App
    { id: 'shortcuts', label: 'Keyboard Shortcuts', category: 'File', icon: <Keyboard size={13} />, shortcut: '⌘/', action: () => setShortcutsOpen(true) },
  ];
  ...
  return (
    <>
      ...
      <div className="adv-ui flex flex-col overflow-hidden"
        ...
        <ImportDialog isOpen={isImportOpen} onClose={() => setIsImportOpen(false)} onLoad={handleLoadConfig} anchorRef={importBtnRef} />
        <KeyboardShortcutsModal isOpen={shortcutsOpen} onClose={() => setShortcutsOpen(false)} />
        <ContextMenu
          state={ctxMenu} onClose={closeCtxMenu} isSelected={ctxBadgeSelected}
        ...
        <ExportPopover
          config={config} onLoadConfig={handleLoadConfig} baseUrl={baseUrl}
        ...
        {/* ── Header ────────────────────────────────────────────────────── */}
        {!isFullscreen && (
          <header className="h-12 shrink-0 flex items-center z-30 relative"
            style={{ background: 'rgba(7,7,6,0.97)', borderBottom: '1px solid rgba(196,124,46,0.08)' }}>
            <div className="absolute bottom-0 left-0 right-0 h-px pointer-events-none"
              style={{ background: 'linear-gradient(90deg, transparent, rgba(196,124,46,0.15), transparent)' }} aria-hidden="true" />
            ...
            <div className="flex items-center px-2 sm:px-3 shrink-0 gap-1.5">
              <a href="/" style={{ textDecoration: 'none', flexShrink: 0 }}>
                <span className="poster-font select-none hidden sm:block"
                  style={{ fontSize: 18, color: 'var(--film-cream)', letterSpacing: '0.12em', lineHeight: 'normal' }}>
                  POSTERIUM
                </span>
                <span className="poster-font select-none sm:hidden"
                  style={{ fontSize: 14, color: 'var(--film-amber)', letterSpacing: '0.12em', lineHeight: 'normal' }}>
                  P
                </span>
              </a>
              {/* "Advanced" badge */}
              <span className="syne-font hidden sm:inline-flex items-center"
                style={{ fontSize: 7, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', padding: '2px 6px', borderRadius: 3, background: 'rgba(196,124,46,0.14)', border: '1px solid rgba(196,124,46,0.28)', color: 'var(--film-amber)' }}>
                Advanced
              </span>
              {/* Switch to standard */}
              <a href="/build"
                className="hidden sm:flex items-center gap-1 hover-cream"
                style={{ fontSize: 9, fontWeight: 600, fontFamily: 'Syne, sans-serif', letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--film-text-dim)', textDecoration: 'none', padding: '3px 8px', borderRadius: 4, border: '1px solid rgba(255,255,255,0.06)', background: 'rgba(255,255,255,0.02)' }}>
                Standard
              </a>
              <ToolbarBtn onClick={() => setShortcutsOpen(v => !v)} label="Keyboard Shortcuts (⌘/)" active={shortcutsOpen} hideOnMobile>
                <Keyboard size={14} />
              </ToolbarBtn>
            </div>
            ...
            <div className="absolute inset-y-0 left-0 right-0 flex items-center justify-center pointer-events-none px-1 sm:px-2">
              <button
                ...
                <span className="text-[11px] syne-font text-left flex-1 min-w-0 truncate">Search commands…</span>
                <kbd className="text-[9px] font-mono px-1.5 py-0.5 rounded border bg-white/5 shrink-0"
                  style={{ borderColor: 'rgba(255,255,255,0.1)' }}>⌘K</kbd>
              </button>
            </div>
            ...
            <div className="ml-auto flex items-center justify-end px-2 sm:px-3 shrink-0 gap-0.5 sm:gap-1">
              ...
              <button
                ...
                <ChevronDown className="max-[1300px]:hidden" size={10} style={{ transform: exportOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.15s' }} />
              </button>
        ...
        <div className="flex flex-1 overflow-hidden relative flex-col lg:flex-row">
          ...
          {/* Canvas */}
          <main
            id="main-canvas" role="main" aria-label="Poster canvas"
            ...
            <PreviewCanvas
              ...
              selectedIds={selectedIds} onSelect={handleSelectionOverride}
            ...
            <div
              className={clsx('fixed z-40 flex items-center gap-1 rounded-xl select-none', isDesktop ? 'flex-col' : 'flex-row')}
              style={{
                ...(isDesktop
                  ? { top: '50%', transform: 'translateY(-50%)', right: (rightVisible && !isFullscreen ? rightW : 0) + 20, transition: 'right 0.3s cubic-bezier(0.16,1,0.3,1)' }
                  : { bottom: 76, right: 12 }),
                background: 'rgba(14,13,11,0.92)', backdropFilter: 'blur(16px)',
                border: '1px solid rgba(196,124,46,0.18)', padding: '6px',
                boxShadow: '0 8px 32px rgba(0,0,0,0.6)',
              }}>
              {[
                { icon: <ZoomIn size={15} />, label: 'Zoom In',    action: () => dispatchZoom(0.25) },
                { icon: <ZoomOut size={15} />, label: 'Zoom Out',  action: () => dispatchZoom(-0.25) },
                ...
                <button key={label} onClick={action} title={label}
                  ...
                  onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = 'var(--film-amber)'; (e.currentTarget as HTMLElement).style.background = 'rgba(196,124,46,0.1)'; }}
          ...
          {!isFullscreen && (
            <aside
              ...
              {/* Resize handle */}
              <div onMouseDown={startResizeRight} className="absolute inset-y-0 left-0 w-2 cursor-col-resize group z-50">
...
const AdvancedBuilderApp: React.FC = () => {
  ...
  return (
    <EditorProvider>
      <AdvancedStudioLayout
        config={config} setConfig={setConfig}
        handleReset={handleReset} baseUrl={baseUrl} handleLoadConfig={handleLoadConfig}
