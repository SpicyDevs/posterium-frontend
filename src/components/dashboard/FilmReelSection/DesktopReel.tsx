...
const CollagePoster = memo<{
  ...
  return (
    <div style={{ width, height, flexShrink: 0, position: 'relative', overflow: 'hidden', background: '#0d0c0a', borderRight: '1px solid rgba(0,0,0,0.7)' }}>
      {!loaded && !err && (
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(110deg,#111009 25%,#1a1712 50%,#111009 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.8s linear infinite' }} />
      )}
      ...
      >
        ...
        <div style={{ flex: 1, position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center' }}>
          ...
          <div style={{ filter: 'sepia(0.18) saturate(0.72) brightness(0.9)' }}>
            <div ref={trackRef} style={{ display: 'flex', flexDirection: 'column', willChange: 'transform', gap: 0 }}>
              {ROWS.map((row, rowIdx) => {
                const { height, count } = ROW_CONFIGS[rowIdx];
                const width = Math.round((height * 2) / 3);
                return (
                  <div key={rowIdx} style={{ display: 'flex', flexDirection: 'row', gap: 0, height, overflow: 'hidden', borderBottom: rowIdx < ROWS.length - 1 ? '2px solid rgba(0,0,0,0.8)' : 'none' }}>
                    {imagesActive
                      ? row.slice(0, count).map((item, idx) => (
                          <CollagePoster key={`r${rowIdx}-${item.id}-${item._slot}`} id={item.id} type={item.type} title={item.title} width={width} height={height} eager={rowIdx === 0 && idx < 6} />
                        ))
                      : Array.from({ length: count }, (_, idx) => (
                          <div key={idx} style={{ width, height, flexShrink: 0, background: 'linear-gradient(110deg,#111009 25%,#1a1712 50%,#111009 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.8s linear infinite', animationDelay: `${(idx % 10) * 0.08}s`, borderRight: '1px solid rgba(0,0,0,0.7)' }} />
                        ))
                    }
                  </div>
                );
