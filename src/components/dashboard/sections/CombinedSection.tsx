...
const ICON_MAP: Record<string, string> = {
  'Drag-Drop Editor': '⌖', 'Instant API URL': '⚡', 'Multiple Sources': '⊞', 'Live Ratings': '◉',
  'Movies, TV & Anime': '▣', 'Any Export Format': '◫', 'Textless Posters': '◻', 'Plex & Jellyfin Ready': '▤',
};

const FEATURE_SRCS: Record<string, string> = Object.fromEntries(
  Object.entries(FEATURE_DEMOS).map(([title, d]) => [
    title, `${API}/${d.type}/${d.id}.svg?r=${d.r}&source=tmdb&blur=7&alpha=0.43&rad=10&${d.pos}`,
  ])
);
...
const ThumbImg = memo<{ title: string; alt: string }>(({ title, alt }) => {
  ...
  if (!src) return null;
  return (
    <div style={{ width: 'clamp(96px,18%,148px)', aspectRatio: '2/3', borderRadius: 4, overflow: 'hidden', border: '1px solid rgba(196,124,46,0.16)', boxShadow: '0 12px 36px rgba(0,0,0,0.7)', flexShrink: 0, background: '#111009', position: 'relative' }}>
      {!loaded && <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(110deg,#111009 25%,#1a1712 50%,#111009 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.6s linear infinite' }} />}
      <img ref={imgRef} src={src} alt={alt} loading="lazy" decoding="async" onLoad={handleLoad} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', opacity: loaded ? 1 : 0, transition: 'opacity 0.4s ease' }} />
    </div>
  );
});
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'clamp(200px,34%,300px) 1fr', borderTop: '1px solid rgba(255,255,255,0.04)', minHeight: 400 }}>
      <div style={{ borderRight: '1px solid rgba(255,255,255,0.04)' }}>
        {FEATURES.map((feat, i) => {
          ...
          return (
            <button key={feat.title} onClick={() => select(i)} style={{ width: '100%', border: 'none', borderBottom: '1px solid rgba(255,255,255,0.028)', borderLeft: isActive ? '2px solid var(--film-amber)' : '2px solid transparent', cursor: 'pointer', textAlign: 'left', padding: '14px 20px', background: isActive ? 'rgba(196,124,46,0.038)' : 'transparent', transition: 'background 0.18s, border-color 0.18s', opacity: vis ? 1 : 0, transform: vis ? 'translateX(0)' : 'translateX(-10px)', transitionProperty: 'background,border-color,opacity,transform', transitionDuration: `0.18s,0.18s,0.5s,0.5s`, transitionDelay: `0s,0s,${i * 0.04}s,${i * 0.04}s`, display: 'flex', alignItems: 'center', gap: 10 }}
              onMouseEnter={(e) => { if (!isActive) (e.currentTarget as HTMLElement).style.background = 'rgba(255,255,255,0.018)'; }}
              onMouseLeave={(e) => { if (!isActive) (e.currentTarget as HTMLElement).style.background = 'transparent'; }}
            >
              <span style={{ fontSize: 14, color: isActive ? 'var(--film-amber)' : 'rgba(122,117,110,0.38)', transition: 'color 0.18s', flexShrink: 0, fontFamily: 'monospace', lineHeight: 1 }}>{ICON_MAP[feat.title] ?? '◆'}</span>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div className="syne-font" style={{ fontSize: 11, fontWeight: 700, color: isActive ? 'var(--film-cream)' : 'rgba(240,230,204,0.6)', transition: 'color 0.18s', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{feat.title}</div>
                <div className="mono-font" style={{ fontSize: 7, letterSpacing: '0.12em', textTransform: 'uppercase', color: isActive ? 'rgba(196,124,46,0.55)' : 'rgba(122,117,110,0.28)', transition: 'color 0.18s', marginTop: 1 }}>{feat.tag}</div>
              </div>
      ...
      <div key={active} style={{ padding: 'clamp(20px,3vw,36px)', display: 'flex', gap: 24, alignItems: 'flex-start', animation: 'fade-up 0.3s ease both' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
            <span className="poster-font" style={{ fontSize: 'clamp(36px,5vw,56px)', lineHeight: 0.9, color: 'rgba(196,124,46,0.1)', letterSpacing: '0.02em', userSelect: 'none' }}>{String(active + 1).padStart(2, '0')}</span>
            <div>
              <span className="mono-font" style={{ fontSize: 7, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'rgba(196,124,46,0.5)', background: 'rgba(196,124,46,0.06)', border: '1px solid rgba(196,124,46,0.14)', borderRadius: 2, padding: '2px 7px', display: 'block', marginBottom: 5 }}>{f.tag}</span>
              <div className="syne-font" style={{ fontSize: 'clamp(14px,2vw,19px)', fontWeight: 800, color: 'var(--film-cream)' }}>{f.title}</div>
            </div>
          </div>
          <p className="body-font" style={{ fontSize: 12, color: 'var(--film-silver)', lineHeight: 1.75, maxWidth: 440, marginBottom: 18 }}>{f.desc}</p>
          ...
          <div style={{ marginTop: 16, display: 'flex', alignItems: 'center', gap: 5 }}>
            ...
            <span className="mono-font" style={{ fontSize: 7, color: '#36A240', letterSpacing: '0.14em' }}>LIVE · FREE · {f.size.toUpperCase()}</span>
          </div>
        </div>
