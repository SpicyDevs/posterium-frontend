...
export const FooterSection = memo(() => (
  ...
  >
    {/* Top sprocket strip */}
    <div style={{ background: 'rgba(255,255,255,0.012)', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
    ...
    <div style={{ position: 'relative', zIndex: 1 }}>
      ...
      <div style={{ padding: 'clamp(56px,8vw,100px) clamp(20px,5vw,64px) 0', position: 'relative' }}>
        ...
        <div
          ...
          style={{
            ...
            fontSize: 'clamp(180px,25vw,360px)', lineHeight: 0.8,
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
            ...
            <div>
              <div className="poster-font" style={{ fontSize: 'clamp(32px,5vw,52px)', color: 'var(--film-cream)', letterSpacing: '0.08em', lineHeight: 0.9 }}>
                POSTERIUM
              </div>
              <div className="mono-font" style={{ fontSize: 8, color: 'rgba(196,124,46,0.5)', letterSpacing: '0.18em', textTransform: 'uppercase', marginTop: 4 }}>
                Free Poster API · MIT License
              </div>
            </div>
          </div>

          <p className="syne-font" style={{ fontSize: 'clamp(12px,1.6vw,16px)', color: 'rgba(110,104,96,0.7)', lineHeight: 1.7, maxWidth: 520, marginBottom: 0 }}>
      ...
      <div style={{ padding: '0 clamp(20px,5vw,64px)', display: 'flex', alignItems: 'center', gap: 0, flexWrap: 'wrap' }}>
        {FOOTER_LINKS.map((link, i) => {
          ...
          const baseStyle: React.CSSProperties = {
            ...
            color: 'rgba(110,104,96,0.55)', fontFamily: 'Syne, sans-serif',
            padding: '5px 0', display: 'inline-flex', alignItems: 'center',
            gap: 4, transition: 'color 0.18s',
          };
          ...
          const el = (
            <a
              ...
              style={baseStyle}
              onMouseEnter={(e) => {
                (e.currentTarget as HTMLElement).style.color = link.internal ? 'var(--film-cream)' : 'var(--film-amber)';
              }}
              onMouseLeave={(e) => {
          ...
          return (
            <span key={link.label} style={{ display: 'inline-flex', alignItems: 'center' }}>
              {el}
              {!isLast && (
                <span aria-hidden="true" style={{ margin: '0 16px', color: 'rgba(196,124,46,0.2)', fontSize: 10, fontFamily: 'monospace' }}>·</span>
              )}
            </span>
          );
        })}
      ...
      {/* Divider */}
      <div aria-hidden="true" style={{ margin: 'clamp(24px,3.5vw,40px) clamp(20px,5vw,64px)', height: 1, background: 'rgba(255,255,255,0.04)' }} />

      {/* Production credits */}
      <div style={{ padding: '0 clamp(20px,5vw,64px) clamp(40px,5vw,60px)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
        <div style={{ display: 'flex', gap: 'clamp(16px,3vw,36px)', flexWrap: 'wrap', alignItems: 'center' }}>
          {[
            ...
            <div key={k} style={{ display: 'flex', alignItems: 'baseline', gap: 5 }}>
              <span className="mono-font" style={{ fontSize: 7, color: 'rgba(196,124,46,0.38)', letterSpacing: '0.18em', textTransform: 'uppercase' }}>{k}</span>
              {v && <span className="syne-font" style={{ fontSize: 9, fontWeight: 700, color: 'rgba(110,104,96,0.45)', letterSpacing: '0.05em' }}>{v}</span>}
            </div>
          ))}
        </div>
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          style={{ display: 'inline-flex', alignItems: 'center', gap: 6, color: 'rgba(196,124,46,0.45)', textDecoration: 'none', fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 9, letterSpacing: '0.08em', textTransform: 'uppercase', transition: 'color 0.18s' }}
    ...
    {/* Bottom sprocket */}
    <div style={{ background: 'rgba(255,255,255,0.012)', borderTop: '1px solid rgba(255,255,255,0.04)' }}>
      <SprocketStrip count={64} />
    </div>

    {/* API URL strip */}
    <div style={{ background: 'rgba(5,5,4,0.95)', padding: '8px clamp(20px,5vw,64px)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
