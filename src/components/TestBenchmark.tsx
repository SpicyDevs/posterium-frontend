...
interface Benchmark {
  ...
  urlKb: number;
  b64Kb: number;
  svgRefCacheStatus: string | null;
  svgB64CacheStatus: string | null;
  bustCache: boolean;
  lb: LbResult;
  nodes: NodeResult[];
  ...
  try {
    ...
    if (!ct.startsWith('image/')) {
      ...
      return {
        ...
        ms,
        status: res.status,
        note: `Non-image (${ct.split(';')[0]}): ${txt.slice(0, 80)}`,
        imageUrl: null,
        ...meta,
      };
    }
...
async function runBenchmark(
  ...
  if (!nodes.length) {
    nodes = NODES.map((n): NodeResult => ({
      ...
      postUrl: {
        ...
        ms: 0,
        status: 0,
        note: serverNote || 'server-side benchmark unavailable',
        imageUrl: null,
      },
      postB64: {
        ok: false,
  ...
  const fastestLabel =
    fastestMs != null
      ? (nodes.find((n) => [n.postUrl, n.postB64].some((r) => r.ok && r.ms === fastestMs))?.label ??
        null)
      : null;
  ...
  return {
    ...
    svgUrlRef,
    svgUrlB64,
    urlKb: serverUrlKb ?? urlKb,
    b64Kb: serverB64Kb ?? b64Kb,
    svgRefCacheStatus,
    svgB64CacheStatus,
    bustCache,
    lb,
...
function IconCacheStatus({ h }: { h: HealthData }) {
  ...
  return (
    <Pill color={C.green}>
      icons: {ic.iconCount} {ageStr && `· ${ageStr}`}
    </Pill>
  );
}
...
function FontStatus({ h }: { h: HealthData }) {
  ...
  const ready =
    h.fontReady ?? (h.fontFiles ? h.fontFiles.length > 0 : h.fontDefault ? true : undefined);
  if (ready === undefined) return <Pill color={C.ghost}>font: ?</Pill>;
  return (
    <Pill color={ready ? C.green : C.red}>
      font: {ready ? h.fontDefault?.split(',')[0] || '✓' : '✗'}
    </Pill>
  );
}
  ...
  return (
    <div style={{ flex: '1 1 0', minWidth: 0, display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
        ...
        {result.cacheStatus && (
          <Pill color={result.cacheStatus === 'HIT' ? C.green : C.orange}>
            {result.cacheStatus}
          </Pill>
        )}
...
function NodeCard({ node, urlKb, b64Kb }: { node: NodeResult; urlKb: number; b64Kb: number }) {
  ...
  return (
    <div
      style={{
        background: C.char,
        border: `1px solid ${hasAny ? C.border : 'rgba(248,113,113,0.2)'}`,
        ...
        {delta !== null && (
          <div
            style={{
              ...
              borderRadius: 4,
              background:
                Math.abs(delta) > 200 ? 'rgba(224,123,57,0.08)' : 'rgba(255,255,255,0.03)',
              ...
              <>
                ...
                <div
                  style={{
                    ...
                    fontSize: 7,
                    fontWeight: 700,
                    color: msColor(node.getRaster.ms),
...
function SummaryPanel({ bench }: { bench: Benchmark }) {
  const postMs = bench.nodes
    .filter((n) => n.postUrl.ok && !n.postUrl.fallback)
    .map((n) => n.postUrl.ms);
  const b64Ms = bench.nodes
    .filter((n) => n.postB64.ok && !n.postB64.fallback)
    .map((n) => n.postB64.ms);
  const getMs = bench.nodes
    .filter((n) => n.getRaster.ok && !n.getRaster.fallback)
    .map((n) => n.getRaster.ms);
  const fallbackNodes = bench.nodes.filter(
    (n) => n.postUrl.fallback || n.postB64.fallback || n.getRaster.fallback
  );
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: 12 }}>
        <div
          style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 2, marginBottom: 8 }}
        >
          {['Method', 'Avg', 'Min', 'Max', 'Size', 'OK'].map((h) => (
            <span key={h} style={{ ...MONO, fontSize: 6, color: C.ghost, padding: '3px 0' }}>
              {h}
            </span>
          ))}
          {/* spacer for 6 cols */}
          {rows.map((r) => [
            <span
              key={r.label + 'l'}
              style={{ ...MONO, fontSize: 8, color: r.color, gridColumn: '1/4' }}
            >
              {r.label}
            </span>,
          ])}
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 9 }}>
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.label}>
                <td style={{ ...MONO, fontSize: 8, color: r.color, padding: '5px 6px' }}>
                  {r.label}
                </td>
                <td
                  style={{
                ...
                <td style={{ ...MONO, fontSize: 9, color: msColor(min(r.ms)), padding: '5px 6px' }}>
                  {fmtMs(min(r.ms))}
                </td>
                <td style={{ ...MONO, fontSize: 9, color: msColor(max(r.ms)), padding: '5px 6px' }}>
                  {fmtMs(max(r.ms))}
                </td>
                <td
                  style={{
                    ...MONO,
                    fontSize: 8,
                    color: r.kb > 0 ? (r.kb > 20 ? C.orange : C.green) : C.ghost,
                    padding: '5px 6px',
                  }}
                >
                  {r.kb > 0 ? `${r.kb}KB` : '—'}
                ...
                <td
                  style={{ ...MONO, fontSize: 9, color: 'var(--film-cream)', padding: '5px 6px' }}
                >
        ...
        {bench.urlKb > 0 && bench.b64Kb > 0 && (
          ...
          >
            <span style={{ ...MONO, fontSize: 8, color: C.orange }}>
              B64 payload is {bench.b64Kb - bench.urlKb}KB larger (+
              {Math.round((bench.b64Kb / bench.urlKb - 1) * 100)}%)
            </span>
            <span style={{ ...MONO, fontSize: 7, color: C.ghost, marginLeft: 8 }}>
        ...
        {fallbackNodes.length > 0 && (
          ...
          >
            <span style={{ ...MONO, fontSize: 8, color: C.orange }}>
              ⚠ {fallbackNodes.length} node{fallbackNodes.length > 1 ? 's' : ''} used wsrv fallback
      ...
      {nodes.map((n) => {
        ...
        return (
          ...
          >
            ...
            >
              <span style={{ ...MONO, fontSize: 10, fontWeight: 700, color: 'var(--film-cream)' }}>
                {n.label}
              </span>
              ...
              {[
                ...
                { l: 'Version', v: h.version || '—', c: C.ghost },
                {
                  ...
                  v: ic?.loaded
                    ? `✓ ${ic.iconCount}`
                    : ic
                      ? `✗ ${ic.lastError?.slice(0, 20) || 'missing'}`
                      : '—',
                ...
                },
                {
                  ...
                  v: ic?.ageMs
                    ? ic.ageMs < 3600000
                      ? `${Math.round(ic.ageMs / 60000)}m`
                      : `${Math.round(ic.ageMs / 3600000)}h`
                    : '—',
                ...
                },
                {
                  l: 'Font',
                  v:
                    h.fontReady !== undefined
                      ? h.fontReady
                        ? h.fontDefault?.split(',')[0] || '✓'
                        : '✗'
                      : h.fontDefault
                        ? h.fontDefault.split(',')[0]
                        : h.fontFiles?.length
                ...
                {
                  l: 'Workers',
                  v: h.workerCount !== undefined ? String(h.workerCount) : '—',
                  c: C.tan,
                },
                {
                  l: 'Active',
                  v: h.activeJobs !== undefined ? String(h.activeJobs) : '—',
                  c: (h.activeJobs || 0) > 0 ? C.yellow : C.ghost,
                },
                {
                  l: 'Queue',
                  v: h.queuedJobs !== undefined ? String(h.queuedJobs) : '—',
                  c: (h.queuedJobs || 0) > 0 ? C.orange : C.ghost,
                },
                {
                  l: 'Respawns',
                  v: h.pendingRespawns !== undefined ? String(h.pendingRespawns) : '—',
                ...
                },
                {
                  ...
                  v: h.uptime
                    ? h.uptime >= 3600
                      ...
                      : `${Math.floor(h.uptime / 60)}m`
                    : '—',
                  c: C.rust,
                },
                {
                  l: 'Max conc',
                  v: h.maxConcurrent !== undefined ? String(h.maxConcurrent) : '—',
                  c: C.ghost,
                },
                { l: 'Node ID', v: h.node || '—', c: C.dim },
              ].map(({ l, v, c }) => (
                <div key={l} style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                  <span style={{ ...MONO, fontSize: 6, color: C.ghost, letterSpacing: '0.1em' }}>
                    {l}
                  </span>
                  <span
                    style={{
...
function TimingBars({ nodes }: { nodes: NodeResult[] }) {
  const maxMs = Math.max(
    ...nodes.flatMap((n) =>
      [n.postUrl, n.postB64, n.getRaster].filter((r) => r.ok).map((r) => r.ms)
    ),
    1
  );
        ...
        <div key={n.id}>
          ...
          {[
            ...
            >
              ...
              >
                {r.ok && (
                  <div
                    style={{
                      height: '100%',
                      transform: `scaleX(${r.ms / maxMs})`,
                      transformOrigin: 'left',
                      background: msColor(r.ms),
                      borderRadius: 2,
              ...
              {!r.ok && (
                <span style={{ ...MONO, fontSize: 7, color: C.red }}>{r.note.slice(0, 40)}</span>
              )}
            </div>
          ))}
        </div>
...
function SearchForm({
  ...
  onRun: (t: string, i: string, p: string, f: string, bust: boolean) => void;
}) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, maxWidth: 580 }}>
      ...
      >
        ...
        {!bustCache && (
          <span style={{ color: C.orange }}>— off: re-runs hit the Workers Cache (HIT)</span>
        )}
      </label>
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ fontSize: 20, color: C.amber, letterSpacing: '0.06em', marginBottom: 14 }}
      >
        {type}/{id}
      </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <div
          className="poster-font"
          style={{ fontSize: 18, color: C.amber, letterSpacing: '0.06em' }}
        >
      ...
      {bench.serverNote && (
        ...
        >
          ⚠ server-side benchmark partially failed — {bench.serverNote}. Node cards below show
          fallback state.
        </div>
      )}
      ...
      <div
        style={{
          background: C.char,
          border: `1px solid ${bench.lb.ok ? C.border : 'rgba(248,113,113,0.25)'}`,
          borderRadius: 8,
          overflow: 'hidden',
        }}
      >
          ...
          <div style={{ flex: 1, minWidth: 220, display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
              ...
              {bench.lb.cacheStatus && (
                <Pill color={bench.lb.cacheStatus === 'HIT' ? C.green : C.orange}>
                  cf: {bench.lb.cacheStatus}
                </Pill>
              )}
            </div>
      ...
      >
        {[
          {
            l: 'Fastest POST',
            v: fmtMs(bench.summary.fastestMs),
            c: msColor(bench.summary.fastestMs),
          },
          { l: 'Best Node', v: bench.summary.fastestLabel ?? '—', c: C.gold },
          {
            l: 'Nodes OK',
            v: `${bench.summary.successCount}/${NODES.length}`,
            c:
              bench.summary.successCount === 0
                ? C.red
                : bench.summary.successCount < NODES.length
                  ? C.yellow
                  : C.green,
          },
          {
            l: 'LB Attempts',
            v: bench.lb.attempts != null ? String(bench.lb.attempts) : '—',
            c: C.tan,
          },
          ...
          >
            ...
            <div className="poster-font" style={{ fontSize: 20, color: c, lineHeight: 1 }}>
              {v}
            </div>
          </div>
        ))}
        ...
        {SECTIONS.map((s) => (
          <button
            ...
            style={{
              ...
              border: 'none',
              cursor: 'pointer',
              color: section === s.key ? C.amber : C.ghost,
              fontSize: 11,
              fontWeight: 600,
              ...SYNE,
      ...
      {section === 'timing' && (
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 12, marginBottom: 12, flexWrap: 'wrap' }}>
            {[
              { c: C.gold, k: 'POST URL-SVG' },
              { c: C.orange, k: 'POST B64-SVG' },
              { c: C.ghost, k: 'GET ?url=' },
            ].map(({ c, k }) => (
              <span
                key={k}
              ...
              <tbody>
                ...
                >
                  ...
                  <td style={{ padding: '5px 12px' }}>
                    <span style={{ color: bench.lb.ok ? C.green : C.red, fontWeight: 700 }}>
                      {bench.lb.ok ? '✓ OK' : '✗ FAIL'}
                    </span>
                  </td>
                  <td
                    style={{
                      padding: '5px 12px',
                      color: bench.lb.ok ? msColor(bench.lb.ms) : C.ghost,
                      fontWeight: bench.lb.ok ? 700 : 400,
                    }}
                  >
                  ...
                  .map((r, i) => (
                    ...
                    >
                      ...
                      <td
                        style={{
                          padding: '5px 12px',
                          color: r.ok ? msColor(r.ms) : C.ghost,
      ...
      >
        Completed {new Date(bench.timestamp).toLocaleTimeString()} · fleet probes + LB ladder run
        server-side · HTTP nodes never touch the browser (mixed-content-safe) ·
...
export default function TestBenchmark() {
  ...
  const go = useCallback(
    async (type: string, id: string, params: string, format: string, bust: boolean) => {
      cleanupBlobs();
      setMode('loading');
      setError('');
      setBench(null);
      ...
      try {
        ...
        blobUrls.current = result.nodes
          .flatMap((n) => [n.postUrl.imageUrl, n.postB64.imageUrl, n.getRaster.imageUrl])
  ...
  return (
    ...
    >
      ...
      >
        <div
          className="poster-font"
          style={{ fontSize: 15, color: C.amber, letterSpacing: '0.06em' }}
        >
      ...
      <main style={{ padding: 16, maxWidth: 1280, margin: '0 auto' }}>
        ...
        {mode === 'search' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <div>
              ...
              >
                Benchmarks all 8 fleet nodes (<strong>POST URL-SVG</strong> vs{' '}
                <strong>POST B64-SVG</strong>, plus <strong>GET ?url=</strong>) entirely server-side
                — the backend probes each node, so no mixed-content http blocks, CORS failures, or
                opaque redirects. Then pushes the same poster through the
                <strong> production LB ladder</strong> (Worker B via the RASTERIZER binding) to
                verify fallback protection — with <strong>cache-busting</strong> for fresh renders
                and cf-cache-status surfaced on every fetch. Node health merges in from the
                server-side fleet probe. Icons are embedded server-side — nodes never need
                icon-cache network access.
              </p>
            </div>
            <SearchForm onRun={go} />
          </div>
        ...
        {mode === 'results' && bench && (
          <ResultsView
            ...
            onRerun={() =>
              lastRun && go(lastRun.type, lastRun.id, lastRun.params, lastRun.format, lastRun.bust)
            }
          />
        )}
      </main>
