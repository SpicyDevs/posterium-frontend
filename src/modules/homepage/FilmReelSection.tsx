          ...
          <div ref={imgRef} style={{ display: 'flex', height: '100%', width: 'max-content', flexShrink: 0, willChange: 'transform', filter: 'sepia(0.18) saturate(0.72) brightness(0.9)' }}>
            {Array.from({ length: DESKTOP_REEL_CHUNKS }).map((_, i) => (
              <picture key={i} style={{ display: 'block', height: '100%' }}>
                ...
                <img src={`/reel-mosaic-${i + 1}.jpg`} alt={i === 0 ? 'Collage of movie and TV posters with rating badges' : ''} style={{ display: 'block', height: '100%', width: 'auto', maxWidth: 'none' }} loading={i < 2 ? 'eager' : 'lazy'} fetchPriority={i === 0 ? ('high' as any) : 'auto'} decoding="async" />
              </picture>
            ))}
          </div>
        </div>
...
const MobileStaticReel = memo(() => {
  ...
  const setHeight = useCallback(() => {
    ...
    const vw = window.innerWidth;
    const vh = window.innerHeight;
    if (trackW > vw) container.style.height = `${trackW - vw + vh}px`;
    // Cache AFTER setting height
    containerAbsTopRef.current = getAbsTop(container);
  }, [mosaicMobileWidth]);
  ...
  return (
    <div id="reel" ref={containerRef} role="region" aria-label="Film Reel Showcase" style={{ position: 'relative' }}>
      <div style={{
        ...
        <div style={{ flexShrink: 0, background: 'rgba(255,255,255,0.015)', borderBottom: '1px solid rgba(255,255,255,0.045)', height: 24 }}>
          <SprocketStrip count={22} />
        </div>
        <div style={{ flex: 1, position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center' }}>
          <div aria-hidden="true" style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 60, zIndex: 2, pointerEvents: 'none', background: 'linear-gradient(to right, var(--film-dark) 0%, rgba(14,13,11,0.88) 60%, transparent 100%)' }} />
          <div aria-hidden="true" style={{ position: 'absolute', right: 0, top: 0, bottom: 0, width: 80, zIndex: 2, pointerEvents: 'none', background: 'linear-gradient(to left, var(--film-dark) 0%, rgba(14,13,11,0.9) 55%, transparent 100%)' }} />
          <div ref={trackRef} style={{ display: 'flex', height: '100%', width: 'max-content', flexShrink: 0, willChange: 'transform', filter: 'sepia(0.18) saturate(0.72) brightness(0.9)' }}>
            {Array.from({ length: MOBILE_REEL_CHUNKS }).map((_, i) => (
              <picture key={i} style={{ display: 'block', height: '100%' }}>
                <source srcSet={`/reel-mosaic-${i + 1}.webp`} type="image/webp" />
                <img src={`/reel-mosaic-${i + 1}.jpg`} alt={i === 0 ? 'Collage of movie and TV posters with rating badges' : ''} style={{ display: 'block', height: '100%', width: 'auto', maxWidth: 'none' }} loading={i === 0 ? 'eager' : 'lazy'} fetchPriority={i === 0 ? ('high' as any) : 'auto'} decoding="async" />
              </picture>
            ))}
          </div>
        </div>
        <div style={{ flexShrink: 0, background: 'rgba(255,255,255,0.015)', borderTop: '1px solid rgba(255,255,255,0.045)', height: 24 }}>
          <SprocketStrip count={22} />
        </div>
        <div style={{ flexShrink: 0, padding: '6px 16px', borderTop: '1px solid rgba(196,124,46,0.06)', display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.04)', borderRadius: 99, overflow: 'hidden' }}>
            <div ref={progressFillRef} style={{ height: '100%', width: '0%', borderRadius: 99, background: 'linear-gradient(90deg, var(--film-amber), #D4A245)', transition: 'none' }} />
          </div>
        </div>
