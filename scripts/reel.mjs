...
function fillGaps(colHeights, cfg, fillers, rng) {
  ...
  while (iter++ < MAX_ITER) {
    ...
    extraItems.push({
      ...
      h,
      span: 1,
      preset: isMinimal ? "minimal" : "badge",
      apiParams,
    });
    heights[col] = y + h + gap;
  }
...
function buildLayout(posters, fillerPosters, cfg, rng) {
  ...
  while (true) {
    ...
    if (result.canvasHeight >= cfg.minHeight) {
      console.log(
        `  Layout converged in ${pass} pass(es). Canvas height: ${result.canvasHeight}px`,
      );
      break;
    }
    ...
    if (pass > 20) {
      console.warn(
        "  Warning: minHeight not reached after 20 passes. Proceeding anyway.",
      );
      break;
    }
  }

  const { extraPlacements, finalColHeights, fillerCount } = fillGaps(
    result.colHeights,
    cfg,
    fillerPosters,
    rng,
  ...
  if (fillerCount > 0) {
    console.log(
      `  Gap-fill: inserted ${fillerCount} filler poster(s) to patch short columns.`,
    );
  } else {
    console.log(
      "  Gap-fill: all columns within threshold — no fillers needed.",
    );
  }
...
function posterUrl(placement, cfg) {
  ...
  const params =
    cfg.showBadges && placement.apiParams ? `?${placement.apiParams}` : "?v=3";
  return `${base}${params}`;
}
...
async function fetchBuffer(url, retries = 3) {
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      console.warn(
        `    Retry ${attempt + 1}/${retries} in ${wait}ms (${err.message})`,
      );
...
async function main() {
  ...
  console.log(
    `  Canvas      : ${CONFIG.canvasWidth}px wide × ≥${CONFIG.minHeight}px tall`,
  );
  console.log(
    `  Chunks      : ${CONFIG.numChunks} (${CONFIG.chunkWidth}px each)`,
  );
  console.log(`  Columns     : ${CONFIG.numCols}`);
  console.log(
    `  Double-col  : ${Math.round(CONFIG.doubleColChance * 100)}% chance`,
  );
  console.log(`  Gap fill    : ≤${CONFIG.gapFillThreshold}px threshold`);
  console.log(
    `  Badges      : ${CONFIG.showBadges ? "yes (dynamic per poster)" : "no"}`,
  );
  console.log(
    `  Posters     : ${POSTERS.length} main + ${FILLER_POSTERS.length} fillers`,
  );
  console.log("");
  ...
  const { placements, canvasHeight } = buildLayout(
    POSTERS,
    FILLER_POSTERS,
    CONFIG,
    rng,
  ...
  console.log(
    `  Total placements: ${placements.length}  (${CONFIG.canvasWidth}×${canvasHeight}px)`,
  );
  console.log("");
  ...
  console.log(
    `  Composited ${info.width}×${info.height}px  (${info.channels} channels)`,
  );
  console.log("");
  ...
  const meta = {
    ...
    placements: placements.map(
      ({ poster, x, y, w, h, span, preset, apiParams }) => ({
        id: poster.id,
        type: poster.type,
        title: poster.title,
  ...
  console.log("");
  console.log(
    `Done!  ${info.width}×${info.height}px (${CONFIG.numChunks} chunks) · ${placements.length} placements`,
  );
  console.log(
    `Tip: regenerate with a different seed:  MOSAIC_SEED=<n> node scripts/reel.mjs`,
  );
}
